...
from jellyfinapi import BASE_HEADERS, CONFIG, TIMEOUT, log, logfilter
from jellyfinapi import utils
from jellyfinapi.alert import AlertListener
from jellyfinapi.base import JellyfinObject
from jellyfinapi.client import JellyfinClient
from jellyfinapi.collection import Collection
from jellyfinapi.exceptions import BadRequest, NotFound, Unauthorized
from jellyfinapi.library import Hub, Library, Path, File
from jellyfinapi.media import Conversion, Optimized
from jellyfinapi.playlist import Playlist
from jellyfinapi.playqueue import PlayQueue
from jellyfinapi.settings import Settings
from jellyfinapi.utils import deprecated
from requests.status_codes import _codes as codes

# Need these imports to populate utils.PLEXOBJECTS
from jellyfinapi import audio as _audio  # noqa: F401
from jellyfinapi import collection as _collection  # noqa: F401
from jellyfinapi import media as _media  # noqa: F401
from jellyfinapi import photo as _photo  # noqa: F401
from jellyfinapi import playlist as _playlist  # noqa: F401
from jellyfinapi import video as _video  # noqa: F401


class JellyfinServer(JellyfinObject):
    """ This is the main entry point to interacting with a Jellyfin server. It allows you to
        list connected clients, browse your library sections and perform actions such as
        emptying trash. If you do not know the auth token required to access your Jellyfin
        server, or simply want to access your server with your username and password, you
        can also create an JellyfinServer instance from :class:`~jellyfinapi.myjellyfin.MyJellyfinAccount`.

        Parameters:
            baseurl (str): Base url for to access the Jellyfin Media Server (default: 'http://localhost:32400').
            token (str): Required Jellyfin authentication token to access the server.
        ...
        Attributes:
            ...
            hubSearch (bool): True if `Hub Search <https://www.jellyfin.tv/blog
                /seek-jellyfin-shall-find-leveling-web-app/>`_ is enabled. I believe this
                is enabled for everyone
            machineIdentifier (str): Unique ID for this server (looks like an md5).
            multiuser (bool): True if `multiusers <https://support.jellyfin.tv/hc/en-us/articles
                /200250367-Multi-User-Support>`_ are enabled.
            myJellyfin (bool): Unknown (True if logged into myJellyfin?).
            myJellyfinMappingState (str): Unknown (ex: mapped).
            myJellyfinSigninState (str): Unknown (ex: ok).
            myJellyfinSubscription (bool): True if you have a myJellyfin subscription.
            myJellyfinUsername (str): Email address if signed into myJellyfin (user@example.com)
            ownerFeatures (list): List of features allowed by the server owner. This may be based
                on your JellyfinPass subscription. Features include: camera_upload, cloudsync,
            ...
            photoAutoTag (bool): True if photo `auto-tagging <https://support.jellyfin.tv/hc/en-us
            ...
            streamingBrainVersion (bool): Current `Streaming Brain <https://www.jellyfin.tv/blog
                /mcstreamy-brain-take-world-two-easy-steps/>`_ version.
            sync (bool): True if `syncing to a device <https://support.jellyfin.tv/hc/en-us/articles
            ...
            version (str): Current Jellyfin version (ex: 1.3.2.3112-1751929)
    ...
    def __init__(self, baseurl=None, token=None, session=None, timeout=None):
        ...
        self._myJellyfinAccount = None   # cached myJellyfinAccount
        ...
        super(JellyfinServer, self).__init__(self, data, self.key)

    def _loadData(self, data):
        """ Load attribute values from Jellyfin XML response. """
        self._data = data
        self.allowCameraUpload = None
        self.allowChannelAccess = None
        self.allowMediaDeletion = None
        self.allowSharing = None
        self.allowSync = None
        self.backgroundProcessing = None
        self.certificate = None
        self.companionProxy = None
        self.diagnostics = None
        self.eventStream = None
        self.friendlyName = data["ServerName"]
        self.hubSearch = None
        self.machineIdentifier = None
        self.multiuser = None
        self.myJellyfin = None
        self.myJellyfinMappingState = None
        self.myJellyfinSigninState = None
        self.myJellyfinSubscription = None
        self.myJellyfinUsername = None
        self.ownerFeatures = None
        self.photoAutoTag = None
        self.platform = data["OperatingSystemDisplayName"]
        self.platformVersion = None
        self.pluginHost = None
        self.readOnlyLibraries = None
        self.requestParametersInCookie = None
        self.streamingBrainVersion = None
        self.sync = None
        self.transcoderActiveVideoSessions = None
        self.transcoderAudio = None
        self.transcoderLyrics = None
        self.transcoderPhoto = None
        self.transcoderSubtitles = None
        self.transcoderVideo = None
    ...
    def _uriRoot(self):
        return f'server://{self.machineIdentifier}/com.jellyfinapp.plugins.library'
    ...
    def library(self):
        ...
        try:
            ...
            data = self.query('/Library/VirtualFolders')
    ...
    def identity(self):
        """ Returns the Jellyfin server identity. """
        data = self.query('/identity')
        return Identity(self, data)

    def account(self):
        """ Returns the :class:`~jellyfinapi.server.Account` object this server belongs to. """
        data = self.query(Account.key)
        return Account(self, data)

    def claim(self, account):
        """ Claim the Jellyfin server using a :class:`~jellyfinapi.myjellyfin.MyJellyfinAccount`.
            This will only work with an unclaimed server on localhost or the same subnet.

            Parameters:
                account (:class:`~jellyfinapi.myjellyfin.MyJellyfinAccount`): The account used to
                    claim the server.
        """
        key = '/myjellyfin/claim'
        params = {'token': account.claimToken()}
        data = self.query(key, method=self._session.post, params=params)
        return Account(self, data)

    def unclaim(self):
        """ Unclaim the Jellyfin server. This will remove the server from your
            :class:`~jellyfinapi.myjellyfin.MyJellyfinAccount`.
        """
    ...
    def agents(self, mediaType=None):
        """ Returns a list of :class:`~jellyfinapi.media.Agent` objects this server has available. """
        key = '/system/agents'
        if mediaType:
    ...
    def switchUser(self, user, session=None, timeout=None):
        """ Returns a new :class:`~jellyfinapi.server.JellyfinServer` object logged in as the given username.
            Note: Only the admin account can switch to other users.

            Parameters:
                user (:class:`~jellyfinapi.myjellyfin.MyJellyfinUser` or str): `MyJellyfinUser` object, username,
            ...
            Example:
                ...
                .. code-block:: python
                    ...
                    # Login to the Jellyfin server using the admin token
                    jellyfin = JellyfinServer('http://jellyfinserver:32400', token='2ffLuB84dqLswk9skLos')
                    # Login to the same Jellyfin server using a different account
                    userJellyfin = jellyfin.switchUser("Username")

        """
        from jellyfinapi.myjellyfin import MyJellyfinUser
        user = user if isinstance(user, MyJellyfinUser) else self.myJellyfinAccount().user(user)
        userToken = user.get_token(self.machineIdentifier)
        if session is None:
            session = self._session
        if timeout is None:
            timeout = self._timeout
        return JellyfinServer(self._baseurl, token=userToken, session=session, timeout=timeout)

    def systemAccounts(self):
        """ Returns a list of :class:`~jellyfinapi.server.SystemAccount` objects this server contains. """
    ...
    def systemAccount(self, accountID):
        """ Returns the :class:`~jellyfinapi.server.SystemAccount` object for the specified account ID.

            Parameters:
                accountID (int): The :class:`~jellyfinapi.server.SystemAccount` ID.
        """
        try:
    ...
    def systemDevices(self):
        """ Returns a list of :class:`~jellyfinapi.server.SystemDevice` objects this server contains. """
    ...
    def systemDevice(self, deviceID):
        """ Returns the :class:`~jellyfinapi.server.SystemDevice` object for the specified device ID.

            Parameters:
                deviceID (int): The :class:`~jellyfinapi.server.SystemDevice` ID.
        """
        try:
    ...
    def myJellyfinAccount(self):
        """ Returns a :class:`~jellyfinapi.myjellyfin.MyJellyfinAccount` object using the same
            token to access this server. If you are not the owner of this JellyfinServer
            you're likely to receive an authentication error calling this.
        """
        if self._myJellyfinAccount is None:
            from jellyfinapi.myjellyfin import MyJellyfinAccount
            self._myJellyfinAccount = MyJellyfinAccount(token=self._token, session=self._session)
        return self._myJellyfinAccount

    def _myJellyfinClientPorts(self):
        """ Sometimes the JellyfinServer does not properly advertise port numbers required
            to connect. This attempts to look up device port number from jellyfin.tv.
            See issue #126: Make JellyfinServer.clients() more user friendly.
              https://github.com/pkkid/python-jellyfinapi/issues/126
        """
        try:
            ports = {}
            account = self.myJellyfinAccount()
            for device in account.devices():
                if device.connections and ':' in device.connections[0][6:]:
                    ports[device.clientIdentifier] = device.connections[0].split(':')[-1]
            return ports
        except Exception as err:
            log.warning('Unable to fetch client ports from myJellyfin: %s', err)
            return ports

    def browse(self, path=None, includeFiles=True):
        """ Browse the system file path using the Jellyfin API.
            Returns list of :class:`~jellyfinapi.library.Path` and :class:`~jellyfinapi.library.File` objects.

            Parameters:
                path (:class:`~jellyfinapi.library.Path` or str, optional): Full path to browse.
    ...
    def walk(self, path=None):
        """ Walk the system file tree using the Jellyfin API similar to `os.walk`.
            Yields a 3-tuple `(path, paths, files)` where
            `path` is a string of the directory path,
            `paths` is a list of :class:`~jellyfinapi.library.Path` objects, and
            `files` is a list of :class:`~jellyfinapi.library.File` objects.

            Parameters:
                path (:class:`~jellyfinapi.library.Path` or str, optional): Full path to walk.
        """
        paths = []
        files = []
    ...
    def isBrowsable(self, path):
        """ Returns True if the Jellyfin server can browse the given path.

            Parameters:
                path (:class:`~jellyfinapi.library.Path` or str): Full path to browse.
        """
        if isinstance(path, Path):
            path = path.path
    ...
    def clients(self):
        """ Returns list of all :class:`~jellyfinapi.client.JellyfinClient` objects connected to server. """
        items = []
        ports = None
        for elem in self.query('/clients'):
            port = elem.attrib.get('port')
            if not port:
                log.warning('%s did not advertise a port, checking jellyfin.tv.', elem.attrib.get('name'))
                ports = self._myJellyfinClientPorts() if ports is None else ports
                port = ports.get(elem.attrib.get('machineIdentifier'))
            baseurl = f"http://{elem.attrib['host']}:{port}"
            items.append(JellyfinClient(baseurl=baseurl, server=self,
                                    token=self._token, data=elem, connect=False))

        return items

    def client(self, name):
        """ Returns the :class:`~jellyfinapi.client.JellyfinClient` that matches the specified name.

            Parameters:
                name (str): Name of the client to return.

            Raises:
                :exc:`~jellyfinapi.exceptions.NotFound`: Unknown client name.
        """
    ...
    def createCollection(self, title, section, items=None, smart=False, limit=None,
        ...
        """ Creates and returns a new :class:`~jellyfinapi.collection.Collection`.

            Parameters:
                title (str): Title of the collection.
                section (:class:`~jellyfinapi.library.LibrarySection`, str): The library section to create the collection in.
                items (List): Regular collections only, list of :class:`~jellyfinapi.audio.Audio`,
                    :class:`~jellyfinapi.video.Video`, or :class:`~jellyfinapi.photo.Photo` objects to be added to the collection.
                ...
                **kwargs (dict): Smart collections only, additional custom filters to apply to the
                    search results. See :func:`~jellyfinapi.library.LibrarySection.search` for more info.

            Raises:
                :class:`jellyfinapi.exceptions.BadRequest`: When no items are included to create the collection.
                :class:`jellyfinapi.exceptions.BadRequest`: When mixing media types in the collection.

            Returns:
                :class:`~jellyfinapi.collection.Collection`: A new instance of the created Collection.
            ...
            Example:
                ...
                .. code-block:: python
                    ...
                    collection = jellyfin.createCollection(
    ...
    def createPlaylist(self, title, section=None, items=None, smart=False, limit=None,
        ...
        """ Creates and returns a new :class:`~jellyfinapi.playlist.Playlist`.

            Parameters:
                title (str): Title of the playlist.
                section (:class:`~jellyfinapi.library.LibrarySection`, str): Smart playlists and m3u import only,
                    the library section to create the playlist in.
                items (List): Regular playlists only, list of :class:`~jellyfinapi.audio.Audio`,
                    :class:`~jellyfinapi.video.Video`, or :class:`~jellyfinapi.photo.Photo` objects to be added to the playlist.
            ...
            Raises:
                :class:`jellyfinapi.exceptions.BadRequest`: When no items are included to create the playlist.
                :class:`jellyfinapi.exceptions.BadRequest`: When mixing media types in the playlist.
                :class:`jellyfinapi.exceptions.BadRequest`: When attempting to import m3u file into non-music library.
                :class:`jellyfinapi.exceptions.BadRequest`: When failed to import m3u file.

            Returns:
                :class:`~jellyfinapi.playlist.Playlist`: A new instance of the created Playlist.
            ...
            Example:
                ...
                .. code-block:: python
                    ...
                    episodes = jellyfin.library.section("TV Shows").get("Game of Thrones").episodes()
                    playlist = jellyfin.createPlaylist(
                        title="GoT Episodes",
                        items=episodes
                    )
    ...
    def createPlayQueue(self, item, **kwargs):
        """ Creates and returns a new :class:`~jellyfinapi.playqueue.PlayQueue`.

            Parameters:
                item (Media or Playlist): Media or playlist to add to PlayQueue.
                kwargs (dict): See `~jellyfinapi.playqueue.PlayQueue.create`.
        """
    ...
    def butlerTasks(self):
        """ Return a list of :class:`~jellyfinapi.base.ButlerTask` objects. """
    ...
    def runButlerTask(self, task):
        """ Manually run a butler task immediately instead of waiting for the scheduled task to run.
            Note: The butler task is run asynchronously. Check Jellyfin Web to monitor activity.
            ...
            Example:
                ...
                .. code-block:: python
                    ...
                    availableTasks = [task.name for task in jellyfin.butlerTasks()]
    ...
    def checkForUpdate(self, force=True, download=False):
        """ Returns a :class:`~jellyfinapi.server.Release` object containing release info
    ...
    def isLatest(self):
        """ Returns True if the installed version of Jellyfin Media Server is the latest. """
        release = self.checkForUpdate(force=True)
        return release is None

    def canInstallUpdate(self):
        """ Returns True if the newest version of Jellyfin Media Server can be installed automatically.
    ...
    def installUpdate(self):
        """ Automatically install the newest version of Jellyfin Media Server. """
    ...
    def playlists(self, playlistType=None, sectionId=None, title=None, sort=None, **kwargs):
        """ Returns a list of all :class:`~jellyfinapi.playlist.Playlist` objects on the server.
    ...
    def playlist(self, title):
        """ Returns the :class:`~jellyfinapi.client.Playlist` that matches the specified title.
    ...
    def optimizedItems(self, removeAll=None):
        """ Returns list of all :class:`~jellyfinapi.media.Optimized` objects connected to server. """
    ...
    @deprecated('use "jellyfinapi.media.Optimized.items()" instead')
    def optimizedItem(self, optimizedID):
        """ Returns single queued optimized item :class:`~jellyfinapi.media.Video` object.
    ...
    def conversions(self, pause=None):
        """ Returns list of all :class:`~jellyfinapi.media.Conversion` objects connected to server. """
        if pause is True:
    ...
    def currentBackgroundProcess(self):
        """ Returns list of all :class:`~jellyfinapi.media.TranscodeJob` objects running or paused on server. """
        return self.fetchItems('/status/sessions/background')

    def query(self, key, method=None, headers=None, timeout=None, **kwargs):
        """ Main method used to handle HTTPS requests to the Jellyfin server. This method helps
    ...
    def search(self, query, mediatype=None, limit=None, sectionId=None):
        """ Returns a list of media items or filter categories from the resulting
            `Hub Search <https://www.jellyfin.tv/blog/seek-jellyfin-shall-find-leveling-web-app/>`_
            against all items in your Jellyfin library. This searches genres, actors, directors,
    ...
    def transcodeSessions(self):
        """ Returns a list of all active :class:`~jellyfinapi.media.TranscodeSession` objects. """
        return self.fetchItems('/transcode/sessions')

    def startAlertListener(self, callback=None, callbackError=None):
        """ Creates a websocket connection to the Jellyfin Server to optionally receive
            notifications. These often include messages from Jellyfin about media scans
            as well as updates to currently running Transcode Sessions.

            Returns a new :class:`~jellyfinapi.alert.AlertListener` object.
            ...
            Raises:
                :exc:`~jellyfinapi.exception.Unsupported`: Websocket-client not installed.
        """
        ...
        """ Returns the URL for a transcoded image.
            ...
            Parameters:
                imageUrl (str): The URL to the image
                    (eg. returned by :func:`~jellyfinapi.mixins.PosterUrlMixin.thumbUrl`
                    or :func:`~jellyfinapi.mixins.ArtUrlMixin.artUrl`).
    ...
    def url(self, key, includeToken=None):
        ...
        if self._token and (includeToken or self._showSecrets):
            ...
            return f'{self._baseurl}{key}{delim}X-Jellyfin-Token={self._token}'
        return f'{self._baseurl}{key}'

    def refreshSynclist(self):
        """ Force PMS to download new SyncList from Jellyfin.tv. """
    ...
    def refreshSync(self):
        """ Calls :func:`~jellyfinapi.server.JellyfinServer.refreshSynclist` and
            :func:`~jellyfinapi.server.JellyfinServer.refreshContent`, just like the Jellyfin Web UI does when you click 'refresh'.
        """
    ...
    def _allowMediaDeletion(self, toggle=False):
        ...
        if self.allowMediaDeletion and toggle is False:
            log.debug('Jellyfin is currently allowed to delete media. Toggling off.')
        elif self.allowMediaDeletion and toggle is True:
            log.debug('Jellyfin is currently allowed to delete media. Toggle set to allow, exiting.')
            raise BadRequest('Jellyfin is currently allowed to delete media. Toggle set to allow, exiting.')
        elif self.allowMediaDeletion is None and toggle is True:
            log.debug('Jellyfin is currently not allowed to delete media. Toggle set to allow.')
        else:
            log.debug('Jellyfin is currently not allowed to delete media. Toggle set to not allow, exiting.')
            raise BadRequest('Jellyfin is currently not allowed to delete media. Toggle set to not allow, exiting.')
    ...
    def bandwidth(self, timespan=None, **kwargs):
        """ Returns a list of :class:`~jellyfinapi.server.StatisticsBandwidth` objects
            with the Jellyfin server dashboard bandwidth data.
            ...
            Parameters:
                ...
                **kwargs (dict, optional): Any of the available filters that can be applied to the bandwidth data.
                    ...
                    * accountID (int): The :class:`~jellyfinapi.server.SystemAccount` ID to filter.
                    ...
                    * deviceID (int): The :class:`~jellyfinapi.server.SystemDevice` ID to filter.
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.BadRequest`: When applying an invalid timespan or unknown filter.
            ...
            Example:
                ...
                .. code-block:: python
                    ...
                    from jellyfinapi.server import JellyfinServer
                    jellyfin = JellyfinServer('http://localhost:32400', token='xxxxxxxxxxxxxxxxxxxx')
                    ...
                    bandwidthData = jellyfin.bandwidth(timespan='days', **filters)
        ...
        for key, value in kwargs.items():
            ...
            if key.startswith('at'):
                ...
                if value == self.myJellyfinAccount().id:
    ...
    def resources(self):
        """ Returns a list of :class:`~jellyfinapi.server.StatisticsResources` objects
            with the Jellyfin server dashboard resources data. """
...
class Account(JellyfinObject):
    """ Contains the locally cached MyJellyfin account information. The properties provided don't
        match the :class:`~jellyfinapi.myjellyfin.MyJellyfinAccount` object very well. I believe this exists
        because access to myjellyfin is not required to get basic jellyfin information. I can't imagine
        object is terribly useful except unless you were needed this information while offline.

        Parameters:
            server (:class:`~jellyfinapi.server.JellyfinServer`): JellyfinServer this account is connected to (optional)
            data (ElementTree): Response from JellyfinServer used to build this object (optional).

        Attributes:
            authToken (str): Jellyfin authentication token to access the server.
            mappingError (str): Unknown
            mappingErrorMessage (str): Unknown
            mappingState (str): Unknown
            privateAddress (str): Local IP address of the Jellyfin server.
            privatePort (str): Local port of the Jellyfin server.
            publicAddress (str): Public IP address of the Jellyfin server.
            publicPort (str): Public port of the Jellyfin server.
            ...
            subscriptionFeatures (str): List of features allowed by the server for this account.
                This may be based on your JellyfinPass subscription. Features include: camera_upload,
            ...
            username (str): Jellyfin account username (user@example.com).
    """
    key = '/myjellyfin/account'
...
class Activity(JellyfinObject):
    """A currently running activity on the JellyfinServer."""
    key = '/activities'
...
class SystemAccount(JellyfinObject):
...
class SystemDevice(JellyfinObject):
    """ Represents a single system device.
        ...
        Attributes:
            ...
            id (int): The ID of the device (not the same as :class:`~jellyfinapi.myjellyfin.MyJellyfinDevice` ID).
...
class StatisticsBandwidth(JellyfinObject):
    """ Represents a single statistics bandwidth data.

        Attributes:
            TAG (str): 'StatisticsBandwidth'
            accountID (int): The associated :class:`~jellyfinapi.server.SystemAccount` ID.
            at (datetime): Datetime of the bandwidth data.
            bytes (int): The total number of bytes for the specified time span.
            deviceID (int): The associated :class:`~jellyfinapi.server.SystemDevice` ID.
    ...
    def account(self):
        """ Returns the :class:`~jellyfinapi.server.SystemAccount` associated with the bandwidth data. """
        return self._server.systemAccount(self.accountID)

    def device(self):
        """ Returns the :class:`~jellyfinapi.server.SystemDevice` associated with the bandwidth data. """
        return self._server.systemDevice(self.deviceID)


class StatisticsResources(JellyfinObject):
    """ Represents a single statistics resources data.
        ...
        Attributes:
            ...
            hostMemoryUtilization (float): The Jellyfin Media Server CPU usage %.
            processCpuUtilization (float): The system RAM usage %.
            processMemoryUtilization (float): The Jellyfin Media Server RAM usage %.
...
@utils.registerJellyfinObject
class ButlerTask(JellyfinObject):
...
class Identity(JellyfinObject):
    """ Represents a server identity.

        Attributes:
            claimed (bool): True or False if the server is claimed.
            machineIdentifier (str): The Jellyfin server machine identifier.
            version (str): The Jellyfin server version.
    """
