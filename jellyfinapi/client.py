...
from jellyfinapi import BASE_HEADERS, CONFIG, TIMEOUT, log, logfilter, utils
from jellyfinapi.base import JellyfinObject
from jellyfinapi.exceptions import BadRequest, NotFound, Unauthorized, Unsupported
...
class JellyfinClient(JellyfinObject):
    """ Main class for interacting with a Jellyfin client. This class can connect
        directly to the client and control it or proxy commands through your
        Jellyfin Server. To better understand the Jellyfin client API's read this page:
        ...
        Parameters:
            ...
            token (str): X-Jellyfin-Token used for authentication (optional).
        ...
        Attributes:
            ...
            product (str): Client Product (Jellyfin for iOS, etc).
            protocol (str): Always seems ot be 'jellyfin'.
            ...
            _proxyThroughServer (bool): Set to True after calling
                :func:`~jellyfinapi.client.JellyfinClient.proxyThroughServer` (default False).
    """
    TAG = 'Player'
    key = '/resources'
    ...
    def __init__(self, server=None, data=None, initpath=None, baseurl=None,
          ...
          parent=None):
        super(JellyfinClient, self).__init__(server, data, initpath)
    ...
    def connect(self, timeout=None):
        """ Alias of reload as any subsequent requests to this client will be
            ...
            populated from a JellyfinServer.
        """
        if not self.key:
    ...
    def proxyThroughServer(self, value=True, server=None):
        """ Tells this JellyfinClient instance to proxy all future commands through the JellyfinServer.
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.Unsupported`: Cannot use client proxy with unknown server.
        """
        if server:
            self._server = server
    ...
    def query(self, path, method=None, headers=None, timeout=None, **kwargs):
        """ Main method used to handle HTTPS requests to the Jellyfin client. This method helps
    ...
    def sendCommand(self, command, proxy=None, **params):
        """ Convenience wrapper around :func:`~jellyfinapi.client.JellyfinClient.query` to more easily
            ...
            Parameters:
                ...
                proxy (bool): Set True to proxy this command through the JellyfinServer.
                **params (dict): Additional GET parameters to include with the command.

            Raises:
                :exc:`~jellyfinapi.exceptions.Unsupported`: When we detect the client doesn't support this capability.
        """
        command = command.strip('/')
        controller = command.split('/')[0]
        headers = {'X-Jellyfin-Target-Client-Identifier': self.machineIdentifier}
        ...
        # Workaround for ptp. See https://github.com/pkkid/python-jellyfinapi/issues/244
        t = time.time()
        if command == 'timeline/poll':
            self._last_call = t
        elif t - self._last_call >= 80 and self.product in ('ptp', 'Jellyfin Media Player'):
            self._last_call = t
        ...
        try:
            ...
            #   - Jellyfinamp, Jellyfin for Android: `b'OK'`
            #   - Jellyfin for Samsung: `b'<?xml version="1.0"?><Response code="200" status="OK">'`
            if self.product in (
                'Jellyfinamp',
                'Jellyfin for Android (TV)',
                'Jellyfin for Android (Mobile)',
                'Jellyfin for Samsung',
            ):
                return
            raise
    ...
    def url(self, key, includeToken=False):
        ...
        if not self._baseurl:
            raise BadRequest('JellyfinClient object missing baseurl.')
    ...
    def goToMedia(self, media, **params):
        """ Navigate directly to the specified media page.
            ...
            Parameters:
                media (:class:`~jellyfinapi.media.Media`): Media object to navigate to.
                **params (dict): Additional GET parameters to include with the command.
        """
    ...
    def playMedia(self, media, offset=0, **params):
        """ Start playback of the specified media item. See also:
            ...
            Parameters:
                media (:class:`~jellyfinapi.media.Media`): Media item to be played back
                ...
                **params (dict): Optional additional parameters to include in the playback request. See
                    also: https://github.com/jellyfininc/jellyfin-media-player/wiki/Remote-control-API#modified-commands
        """
        server_url = media._server._baseurl.split(':')
...
class ClientTimeline(JellyfinObject):
