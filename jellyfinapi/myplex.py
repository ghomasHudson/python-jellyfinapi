...
from jellyfinapi import (BASE_HEADERS, CONFIG, TIMEOUT, X_PLEX_ENABLE_FAST_CONNECT, X_PLEX_IDENTIFIER,
                     log, logfilter, utils)
from jellyfinapi.base import JellyfinObject
from jellyfinapi.client import JellyfinClient
from jellyfinapi.exceptions import BadRequest, NotFound, Unauthorized, TwoFactorRequired
from jellyfinapi.library import LibrarySection
from jellyfinapi.server import JellyfinServer
from jellyfinapi.sonos import JellyfinSonosClient
from jellyfinapi.sync import SyncItem, SyncList
from requests.status_codes import _codes as codes


class MyJellyfinAccount(JellyfinObject):
    """ MyJellyfin account and profile information. This object represents the data found Account on
        the myjellyfin.tv servers at the url https://jellyfin.tv/api/v2/user. You may create this object
        directly by passing in your username & password (or token). There is also a convenience
        method provided at :class:`~jellyfinapi.server.JellyfinServer.myJellyfinAccount()` which will create
        and return this object.

        Parameters:
            username (str): Jellyfin login username if not using a token.
            password (str): Jellyfin login password if not using a token.
            token (str): Jellyfin authentication token instead of username and password.
            session (requests.Session, optional): Use your own session object if you want to
                cache the http responses from PMS.
            timeout (int): timeout in seconds on initial connect to myjellyfin (default config.TIMEOUT).
        ...
        Attributes:
            key (str): 'https://jellyfin.tv/api/v2/user'
            ...
            guest (bool): If the account is a Jellyfin Home guest user.
            hasPassword (bool): If the account has a password.
            home (bool): If the account is a Jellyfin Home user.
            homeAdmin (bool): If the account is the Jellyfin Home admin.
            homeSize (int): The number of accounts in the Jellyfin Home.
            id (int): The Jellyfin account ID.
            joinedAt (datetime): Date the account joined Jellyfin.
            locale (str): the account locale
            mailingListActive (bool): If you are subscribed to the Jellyfin newsletter.
            mailingListStatus (str): Your current mailing list status.
            maxHomeSize (int): The maximum number of accounts allowed in the Jellyfin Home.
            pin (str): The hashed Jellyfin Home PIN.
            ...
            protected (bool): If the account has a Jellyfin Home PIN enabled.
            rememberExpiresAt (datetime): Date the token expires.
            restricted (bool): If the account is a Jellyfin Home managed user.
            roles: (List<str>) Lit of account roles. Jellyfinpass membership listed here.
            scrobbleTypes (List<int>): Unknown.
            subscriptionActive (bool): If the account's Jellyfin Pass subscription is active.
            subscriptionDescription (str): Description of the Jellyfin Pass subscription.
            subscriptionFeatures: (List<str>) List of features allowed on your Jellyfin Pass subscription.
            subscriptionPaymentService (str): Payment service used for your Jellyfin Pass subscription.
            subscriptionPlan (str): Name of Jellyfin Pass subscription plan.
            subscriptionStatus (str): String representation of ``subscriptionActive``.
            subscriptionSubscribedAt (datetime): Date the account subscribed to Jellyfin Pass.
    ...
    """
    FRIENDINVITE = 'https://jellyfin.tv/api/servers/{machineId}/shared_servers'                     # post with data
    HOMEUSERS = 'https://jellyfin.tv/api/home/users'
    HOMEUSERCREATE = 'https://jellyfin.tv/api/home/users?title={title}'                             # post with data
    EXISTINGUSER = 'https://jellyfin.tv/api/home/users?invitedEmail={username}'                     # post with data
    FRIENDSERVERS = 'https://jellyfin.tv/api/servers/{machineId}/shared_servers/{serverId}'         # put with data
    PLEXSERVERS = 'https://jellyfin.tv/api/servers/{machineId}'                                     # get
    FRIENDUPDATE = 'https://jellyfin.tv/api/friends/{userId}'                                       # put with args, delete
    HOMEUSER = 'https://jellyfin.tv/api/home/users/{userId}'                                        # delete, put
    MANAGEDHOMEUSER = 'https://jellyfin.tv/api/v2/home/users/restricted/{userId}'                   # put
    SIGNIN = 'https://jellyfin.tv/api/v2/users/signin'                                              # post with auth
    SIGNOUT = 'https://jellyfin.tv/api/v2/users/signout'                                            # delete
    WEBHOOKS = 'https://jellyfin.tv/api/v2/user/webhooks'                                           # get, post with data
    OPTOUTS = 'https://jellyfin.tv/api/v2/user/{userUUID}/settings/opt_outs'                        # get
    LINK = 'https://jellyfin.tv/api/v2/pins/link'                                                   # put
    VIEWSTATESYNC = 'https://jellyfin.tv/api/v2/user/view_state_sync'                               # put
    PING = 'https://jellyfin.tv/api/v2/ping'
    # Hub sections
    VOD = 'https://vod.provider.jellyfin.tv'                                                        # get
    MUSIC = 'https://music.provider.jellyfin.tv'                                                    # get
    DISCOVER = 'https://discover.provider.jellyfin.tv'
    METADATA = 'https://metadata.provider.jellyfin.tv'
    key = 'https://jellyfin.tv/api/v2/user'
    ...
    def __init__(self, username=None, password=None, token=None, session=None, timeout=None, code=None, remember=True):
        ...
        super(MyJellyfinAccount, self).__init__(self, data, initpath)

    def _signin(self, username, password, code, remember, timeout):
        if self._token:
            return self.query(self.key), self.key
        payload = {
            'login': username or CONFIG.get('auth.myjellyfin_username'),
            'password': password or CONFIG.get('auth.myjellyfin_password'),
            'rememberMe': remember
        }
        if code:
    ...
    def signout(self):
        """ Sign out of the Jellyfin account. Invalidates the authentication token. """
    ...
    def _loadData(self, data):
        ...
        # TODO: Fetch missing MyJellyfinAccount services
        self.services = None
    ...
    def _headers(self, **kwargs):
        ...
        if self._token:
            headers['X-Jellyfin-Token'] = self._token
    ...
    def device(self, name=None, clientId=None):
        """ Returns the :class:`~jellyfinapi.myjellyfin.MyJellyfinDevice` that matches the name specified.
    ...
    def devices(self):
        """ Returns a list of all :class:`~jellyfinapi.myjellyfin.MyJellyfinDevice` objects connected to the server. """
        data = self.query(MyJellyfinDevice.key)
        return [MyJellyfinDevice(self, elem) for elem in data]

    def resource(self, name):
        """ Returns the :class:`~jellyfinapi.myjellyfin.MyJellyfinResource` that matches the name specified.
    ...
    def resources(self):
        """ Returns a list of all :class:`~jellyfinapi.myjellyfin.MyJellyfinResource` objects connected to the server. """
        data = self.query(MyJellyfinResource.key)
        return [MyJellyfinResource(self, elem) for elem in data]
    ...
    def sonos_speakers(self):
        ...
        if t - self._sonos_cache_timestamp > 5:
            ...
            data = self.query('https://sonos.jellyfin.tv/resources')
            self._sonos_cache = [JellyfinSonosClient(self, elem) for elem in data]
    ...
    def inviteFriend(self, user, server, sections=None, allowSync=False, allowCameraUpload=False,
        ...
        """ Share library content with the specified user.
            ...
            Parameters:
                user (:class:`~jellyfinapi.myjellyfin.MyJellyfinUser`): `MyJellyfinUser` object, username, or email
                    of the user to be added.
                server (:class:`~jellyfinapi.server.JellyfinServer`): `JellyfinServer` object, or machineIdentifier
                    containing the library sections to share.
                sections (List<:class:`~jellyfinapi.library.LibrarySection`>): List of `LibrarySection` objects, or names
                ...
                filterMusic (Dict): Dict containing key 'label' set to a list of values to be filtered.
                    ex: `{'label':['foo']}`
        """
        username = user.username if isinstance(user, MyJellyfinUser) else user
        machineId = server.machineIdentifier if isinstance(server, JellyfinServer) else server
    ...
    def createExistingUser(self, user, server, sections=None, allowSync=False, allowCameraUpload=False,
        ...
        if isinstance(user, MyJellyfinUser):
    ...
    def removeFriend(self, user):
        """ Remove the specified user from your friends.
            ...
            Parameters:
                user (:class:`~jellyfinapi.myjellyfin.MyJellyfinUser` or str): :class:`~jellyfinapi.myjellyfin.MyJellyfinUser`,
                    username, or email of the user to be removed.
        """
        user = user if isinstance(user, MyJellyfinUser) else self.user(user)
    ...
    def switchHomeUser(self, user, pin=None):
        """ Returns a new :class:`~jellyfinapi.myjellyfin.MyJellyfinAccount` object switched to the given home user.
            ...
            Example:
                ...
                .. code-block:: python
                    ...
                    # Login to a Jellyfin Home account
                    account = MyJellyfinAccount('<USERNAME>', '<PASSWORD>')
                    # Switch to a different Jellyfin Home user
        ...
        return MyJellyfinAccount(token=userToken, session=self._session)

    def setPin(self, newPin, currentPin=None):
        """ Set a new Jellyfin Home PIN for the account.
    ...
    def removePin(self, currentPin):
        """ Remove the Jellyfin Home PIN for the account.
    ...
    def setManagedUserPin(self, user, newPin):
        """ Set a new Jellyfin Home PIN for a managed home user. This must be done from the Jellyfin Home admin account.
    ...
    def removeManagedUserPin(self, user):
        """ Remove the Jellyfin Home PIN for a managed home user. This must be done from the Jellyfin Home admin account.
    ...
    def acceptInvite(self, user):
        """ Accept a pending friend invite from the specified user.
            ...
            Parameters:
                user (:class:`~jellyfinapi.myjellyfin.MyJellyfinInvite` or str): :class:`~jellyfinapi.myjellyfin.MyJellyfinInvite`,
                    username, or email of the friend invite to accept.
        """
        invite = user if isinstance(user, MyJellyfinInvite) else self.pendingInvite(user, includeSent=False)
        params = {
            'friend': int(invite.friend),
            'home': int(invite.home),
            'server': int(invite.server)
        }
        url = MyJellyfinInvite.REQUESTS + f'/{invite.id}' + utils.joinArgs(params)
    ...
    def cancelInvite(self, user):
        ...
        """
        invite = user if isinstance(user, MyJellyfinInvite) else self.pendingInvite(user, includeReceived=False)
        params = {
            'friend': int(invite.friend),
            'home': int(invite.home),
            'server': int(invite.server)
        }
        url = MyJellyfinInvite.REQUESTED + f'/{invite.id}' + utils.joinArgs(params)
        return self.query(url, self._session.delete)
    ...
    def user(self, username):
        """ Returns the :class:`~jellyfinapi.myjellyfin.MyJellyfinUser` that matches the specified username or email.
    ...
    def users(self):
        """ Returns a list of all :class:`~jellyfinapi.myjellyfin.MyJellyfinUser` objects connected to your account.
        """
        elem = self.query(MyJellyfinUser.key)
        return self.findItems(elem, cls=MyJellyfinUser)

    def pendingInvite(self, username, includeSent=True, includeReceived=True):
        """ Returns the :class:`~jellyfinapi.myjellyfin.MyJellyfinInvite` that matches the specified username or email.
    ...
    def pendingInvites(self, includeSent=True, includeReceived=True):
        """ Returns a list of all :class:`~jellyfinapi.myjellyfin.MyJellyfinInvite` objects connected to your account.
        ...
        invites = []
        if includeSent:
            elem = self.query(MyJellyfinInvite.REQUESTED)
            invites += self.findItems(elem, cls=MyJellyfinInvite)
        if includeReceived:
            elem = self.query(MyJellyfinInvite.REQUESTS)
            invites += self.findItems(elem, cls=MyJellyfinInvite)
        return invites
    ...
    def _getSectionIds(self, server, sections):
        ...
        allSectionIds = {}
        machineIdentifier = server.machineIdentifier if isinstance(server, JellyfinServer) else server
    ...
    def optOut(self, playback=None, library=None):
        """ Opt in or out of sharing stuff with jellyfin.
            See: https://www.jellyfin.tv/about/privacy-legal/
        """
        params = {}
        ...
        url = 'https://jellyfin.tv/api/v2/user/privacy'
        return self.query(url, method=self._session.put, data=params)

    def syncItems(self, client=None, clientId=None):
        """ Returns an instance of :class:`~jellyfinapi.sync.SyncList` for specified client.

            Parameters:
                client (:class:`~jellyfinapi.myjellyfin.MyJellyfinDevice`): a client to query SyncItems for.
    ...
    def sync(self, sync_item, client=None, clientId=None):
        """ Adds specified sync item for the client. It's always easier to use methods defined directly in the media
            objects, e.g. :func:`~jellyfinapi.video.Video.sync`, :func:`~jellyfinapi.audio.Audio.sync`.

            Parameters:
                client (:class:`~jellyfinapi.myjellyfin.MyJellyfinDevice`): a client for which you need to add SyncItem to.
                clientId (str): an identifier of a client for which you need to add SyncItem to.
                sync_item (:class:`~jellyfinapi.sync.SyncItem`): prepared SyncItem object with all fields set.
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.BadRequest`: When client with provided clientId wasn't found.
                :exc:`~jellyfinapi.exceptions.BadRequest`: Provided client doesn't provides `sync-target`.
        """
    ...
    def claimToken(self):
        """ Returns a str, a new "claim-token", which you can use to register your new Jellyfin Server instance to your
            account.
            See: https://hub.docker.com/r/jellyfininc/pms-docker/, https://www.jellyfin.tv/claim/
        """
        response = self._session.get('https://jellyfin.tv/api/claim/token.json', headers=self._headers(), timeout=TIMEOUT)
        if response.status_code not in (200, 201, 204):  # pragma: no cover
            codename = codes.get(response.status_code)[0]
            errtext = response.text.replace('\n', ' ')
    ...
    def onlineMediaSources(self):
        """ Returns a list of user account Online Media Sources settings :class:`~jellyfinapi.myjellyfin.AccountOptOut`
        """
    ...
    def videoOnDemand(self):
        """ Returns a list of VOD Hub items :class:`~jellyfinapi.library.Hub`
        """
        data = self.query(f'{self.VOD}/hubs')
        return self.findItems(data)

    def tidal(self):
        """ Returns a list of tidal Hub items :class:`~jellyfinapi.library.Hub`
        """
        data = self.query(f'{self.MUSIC}/hubs')
        return self.findItems(data)

    def watchlist(self, filter=None, sort=None, libtype=None, maxresults=None, **kwargs):
        """ Returns a list of :class:`~jellyfinapi.video.Movie` and :class:`~jellyfinapi.video.Show` items in the user's watchlist.
            Note: The objects returned are from Jellyfin's online metadata. To get the matching item on a Jellyfin server,
            ...
            Example:
                ...
                .. code-block:: python
                    ...
                    # Search for the item on a Jellyfin server
                    result = jellyfin.library.search(guid=item.guid, libtype=item.type)
    ...
    def onWatchlist(self, item):
        """ Returns True if the item is on the user's watchlist.
            ...
            Parameters:
                item (:class:`~jellyfinapi.video.Movie` or :class:`~jellyfinapi.video.Show`): Item to check
    ...
    def addToWatchlist(self, items):
        """ Add media items to the user's watchlist
            ...
            Parameters:
                items (List): List of :class:`~jellyfinapi.video.Movie` or :class:`~jellyfinapi.video.Show`
                    objects to be added to the watchlist.

            Raises:
                :exc:`~jellyfinapi.exceptions.BadRequest`: When trying to add invalid or existing
    ...
    def removeFromWatchlist(self, items):
        """ Remove media items from the user's watchlist
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.BadRequest`: When trying to remove invalid or non-existing
                    media to the watchlist.
        """
        if not isinstance(items, list):
            items = [items]
    ...
    def userState(self, item):
        """ Returns a :class:`~jellyfinapi.myjellyfin.UserState` object for the specified item.

            Parameters:
                item (:class:`~jellyfinapi.video.Movie` or :class:`~jellyfinapi.video.Show`): Item to return the user state.
        """
        ratingKey = item.guid.rsplit('/', 1)[-1]
    ...
    def isPlayed(self, item):
        """ Return True if the item is played on Discover.
            ...
            Parameters:
                item (:class:`~jellyfinapi.video.Movie`,
                :class:`~jellyfinapi.video.Show`, :class:`~jellyfinapi.video.Season` or
                :class:`~jellyfinapi.video.Episode`): Object from searchDiscover().
                Can be also result from Jellyfin Movie or Jellyfin TV Series agent.
        """
        userState = self.userState(item)
        return bool(userState.viewCount > 0) if userState.viewCount else False

    def markPlayed(self, item):
        """ Mark the Jellyfin object as played on Discover.
        ...
        ratingKey = item.guid.rsplit('/', 1)[-1]
        params = {'key': ratingKey, 'identifier': 'com.jellyfinapp.plugins.library'}
        self.query(key, params=params)
        return self

    def markUnplayed(self, item):
        """ Mark the Jellyfin object as unplayed on Discover.
    ...
    def searchDiscover(self, query, limit=30, libtype=None):
        """ Search for movies and TV shows in Discover.
            Returns a list of :class:`~jellyfinapi.video.Movie` and :class:`~jellyfinapi.video.Show` objects.
    ...
    def link(self, pin):
        ...
        headers = {
            ...
            'X-Jellyfin-Product': 'Jellyfin SSO'
        }
        data = {'code': pin}
    ...
    def _toOnlineMetadata(self, objs, **kwargs):
        ...
        # TODO: Add proper support for metadata.provider.jellyfin.tv
        # Temporary workaround to allow reloading and browsing of online media objects
        server = JellyfinServer(self.METADATA, self._token, session=self._session)
    ...
    def publicIP(self):
        ...
        return self.query('https://jellyfin.tv/:/ip')

    def geoip(self, ip_address):
        """ Returns a :class:`~jellyfinapi.myjellyfin.GeoLocation` object with geolocation information
            for an IP address using Jellyfin's GeoIP database.

            Parameters:
                ip_address (str): IP address to lookup.
        """
        params = {'ip_address': ip_address}
        data = self.query('https://jellyfin.tv/api/v2/geoip', params=params)
        return GeoLocation(self, data)


class MyJellyfinUser(JellyfinObject):
    """ This object represents non-signed in users such as friends and linked
        accounts. NOTE: This should not be confused with the :class:`~jellyfinapi.myjellyfin.MyJellyfinAccount`
        which is your specific account. The raw xml for the data presented here
        can be found at: https://jellyfin.tv/api/users/

        Attributes:
            TAG (str): 'User'
            key (str): 'https://jellyfin.tv/api/users/'
    ...
    def _loadData(self, data):
        ...
        self.title = data.attrib.get('title', '')
        self.username = data.attrib.get('username', '')
        self.servers = self.findItems(data, MyJellyfinServerShare)
    ...
    def server(self, name):
        """ Returns the :class:`~jellyfinapi.myjellyfin.MyJellyfinServerShare` that matches the name specified.
...
class MyJellyfinInvite(JellyfinObject):
    """ This object represents pending friend invites.
        ...
        Attributes:
            ...
            home (bool): True or False if the user is invited to a Jellyfin Home.
            id (int): User's Jellyfin account ID.
    ...
    """
    TAG = 'Invite'
    REQUESTS = 'https://jellyfin.tv/api/invites/requests'
    REQUESTED = 'https://jellyfin.tv/api/invites/requested'
...
class Section(JellyfinObject):
    """ This refers to a shared section. The raw xml for the data presented here
        can be found at: https://jellyfin.tv/api/servers/{machineId}/shared_servers
...
class MyJellyfinServerShare(JellyfinObject):
    ...
    def section(self, name):
        """ Returns the :class:`~jellyfinapi.myjellyfin.Section` that matches the name specified.
    ...
    def sections(self):
        """ Returns a list of all :class:`~jellyfinapi.myjellyfin.Section` objects shared with this user.
        """
        url = MyJellyfinAccount.FRIENDSERVERS.format(machineId=self.machineIdentifier, serverId=self.id)
...
class MyJellyfinResource(JellyfinObject):
    """ This object represents resources connected to your Jellyfin server that can provide
        content such as Jellyfin Media Servers, iPhone or Android clients, etc. The raw xml
        for the data presented here can be found at:
        https://jellyfin.tv/api/v2/resources?includeHttps=1&includeRelay=1

        Attributes:
            TAG (str): 'Device'
            key (str): 'https://jellyfin.tv/api/v2/resources?includeHttps=1&includeRelay=1'
            accessToken (str): This resource's Jellyfin access token.
            clientIdentifier (str): Unique ID for this resource.
            connections (list): List of :class:`~jellyfinapi.myjellyfin.ResourceConnection` objects
                for this resource.
            ...
            product (str): Jellyfin product (Jellyfin Media Server, Jellyfin for iOS, Jellyfin Web, etc.)
            ...
            relay (bool): True if this resource has the Jellyfin Relay enabled.
    ...
    """
    TAG = 'resource'
    key = 'https://jellyfin.tv/api/v2/resources?includeHttps=1&includeRelay=1'
    ...
    def connect(
        ...
        """ Returns a new :class:`~jellyfinapi.server.JellyfinServer` or :class:`~jellyfinapi.client.JellyfinClient` object.
            Uses `MyJellyfinResource.preferred_connections()` to generate the priority order of connection addresses.
            After trying to connect to all available addresses for this resource and
            assuming at least one connection was successful, the JellyfinServer object is built and returned.
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.NotFound`: When unable to connect to any addresses for this resource.
        """
        if locations is None:
            locations = self.DEFAULT_LOCATION_ORDER[:]
        if schemes is None:
        ...
        cls = JellyfinServer if 'server' in self.provides else JellyfinClient
...
class ResourceConnection(JellyfinObject):
    """ Represents a Resource Connection object found within the
        :class:`~jellyfinapi.myjellyfin.MyJellyfinResource` objects.
        ...
        Attributes:
            ...
            relay (bool): True if the address uses the Jellyfin Relay
...
class MyJellyfinDevice(JellyfinObject):
    """ This object represents resources connected to your Jellyfin server that provide
        playback ability from your Jellyfin Server, iPhone or Android clients, Jellyfin Web,
        this API, etc. The raw xml for the data presented here can be found at:
        https://jellyfin.tv/devices.xml

        Attributes:
            TAG (str): 'Device'
            key (str): 'https://jellyfin.tv/devices.xml'
            clientIdentifier (str): Unique ID for this resource.
            ...
            id (str): MyJellyfin ID of the device.
    ...
    def connect(self, timeout=None):
        """ Returns a new :class:`~jellyfinapi.client.JellyfinClient` or :class:`~jellyfinapi.server.JellyfinServer`
            ...
            at least one connection was successful, the JellyfinClient object is built and returned.

            Raises:
                :exc:`~jellyfinapi.exceptions.NotFound`: When unable to connect to any addresses for this device.
        """
        cls = JellyfinServer if 'server' in self.provides else JellyfinClient
    ...
    def delete(self):
        ...
        key = f'https://jellyfin.tv/devices/{self.id}.xml'
        self._server.query(key, self._server._session.delete)

    def syncItems(self):
        """ Returns an instance of :class:`~jellyfinapi.sync.SyncList` for current device.

            Raises:
                :exc:`~jellyfinapi.exceptions.BadRequest`: when the device doesn't provides `sync-target`.
        """
...
class MyJellyfinPinLogin:
    """
        MyJellyfin PIN login class which supports getting the four character PIN which the user must
        enter on https://jellyfin.tv/link to authenticate the client and provide an access token to
        create a :class:`~jellyfinapi.myjellyfin.MyJellyfinAccount` instance.
        ...
        - The polling approach expects the developer to periodically check if the PIN login was
          successful using :func:`~jellyfinapi.myjellyfin.MyJellyfinPinLogin.checkLogin`.
        - The threaded approach expects the developer to call
          :func:`~jellyfinapi.myjellyfin.MyJellyfinPinLogin.run` and then at a later time call
          :func:`~jellyfinapi.myjellyfin.MyJellyfinPinLogin.waitForLogin` to wait for and check the result.
        - The callback approach is an extension of the threaded approach and expects the developer
          to pass the `callback` parameter to the call to :func:`~jellyfinapi.myjellyfin.MyJellyfinPinLogin.run`.
        ...
        Parameters:
            session (requests.Session, optional): Use your own session object if you want to
                cache the http responses from PMS
            requestTimeout (int): timeout in seconds on initial connect to jellyfin.tv (default config.TIMEOUT).
            headers (dict): A dict of X-Jellyfin headers to send with requests.
            oauth (bool): True to use Jellyfin OAuth instead of PIN login.

        Attributes:
            PINS (str): 'https://jellyfin.tv/api/v2/pins'
            CHECKPINS (str): 'https://jellyfin.tv/api/v2/pins/{pinid}'
            POLLINTERVAL (int): 1
            ...
            pin (str): Pin to use for the login on https://jellyfin.tv/link.
    """
    PINS = 'https://jellyfin.tv/api/v2/pins'               # get
    CHECKPINS = 'https://jellyfin.tv/api/v2/pins/{pinid}'  # get
    POLLINTERVAL = 1

    def __init__(self, session=None, requestTimeout=None, headers=None, oauth=False):
        super(MyJellyfinPinLogin, self).__init__()
        self._session = session or requests.Session()
    ...
    def pin(self):
        """ Return the 4 character PIN used for linking a device at
            https://jellyfin.tv/link.
        """
        if self._oauth:
            raise BadRequest('Cannot use PIN for Jellyfin OAuth login')
        return self._code

    def oauthUrl(self, forwardUrl=None):
        """ Return the Jellyfin OAuth url for login.

            Parameters:
                forwardUrl (str, optional): The url to redirect the client to after login.
        """
        if not self._oauth:
            raise BadRequest('Must use "MyJellyfinPinLogin(oauth=True)" for Jellyfin OAuth login.')

        headers = self._headers()
        params = {
            'clientID': headers['X-Jellyfin-Client-Identifier'],
            'context[device][product]': headers['X-Jellyfin-Product'],
            'context[device][version]': headers['X-Jellyfin-Version'],
            'context[device][platform]': headers['X-Jellyfin-Platform'],
            'context[device][platformVersion]': headers['X-Jellyfin-Platform-Version'],
            'context[device][device]': headers['X-Jellyfin-Device'],
            'context[device][deviceName]': headers['X-Jellyfin-Device-Name'],
            'code': self._code
        }
        if forwardUrl:
            params['forwardUrl'] = forwardUrl

        return f'https://app.jellyfin.tv/auth/#!?{urlencode(params)}'
    ...
    def run(self, callback=None, timeout=None):
        ...
        if self._thread and not self._abort:
            raise RuntimeError('MyJellyfinPinLogin thread is already running')
        if self.expired:
            raise RuntimeError('MyJellyfinPinLogin has expired')

        self._loginTimeout = timeout
        self._callback = callback
        self._abort = False
        self.finished = False
        self._thread = threading.Thread(target=self._pollLogin, name='jellyfinapi.myjellyfin.MyJellyfinPinLogin')
        self._thread.start()
    ...
    """ Connects to the specified cls with url and token. Stores the connection
        ...
        Arguments:
            cls: the class which is responsible for establishing connection, basically it's
                 :class:`~jellyfinapi.client.JellyfinClient` or :class:`~jellyfinapi.server.JellyfinServer`
...
def _chooseConnection(ctype, name, results):
    ...
    # At this point we have a list of result tuples containing (url, token, JellyfinServer, runtime)
    ...
    for url, token, result, runtime in results:
        ...
        log.debug('%s connection %s (%ss): %s?X-Jellyfin-Token=%s', ctype, okerr, runtime, url, token)
    results = [r[2] for r in results if r and r[2] is not None]
    if results:
        log.debug('Connecting to %s: %s?X-Jellyfin-Token=%s', ctype, results[0]._baseurl, results[0]._token)
        return results[0]
    raise NotFound(f'Unable to connect to {ctype.lower()}: {name}')


class AccountOptOut(JellyfinObject):
    """ Represents a single AccountOptOut
        'https://jellyfin.tv/api/v2/user/{userUUID}/settings/opt_outs'
    ...
    def _updateOptOut(self, option):
        """ Sets the Online Media Sources option.
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.NotFound`: ``option`` str not found in CHOICES.
        """
    ...
    def optOutManaged(self):
        """ Sets the Online Media Source to "Disabled for Managed Users".
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.BadRequest`: When trying to opt out music.
        """
        if self.key == 'tv.jellyfin.provider.music':
            raise BadRequest(f'{self.key} does not have the option to opt out managed users.')
        self._updateOptOut('opt_out_managed')


class UserState(JellyfinObject):
...
class GeoLocation(JellyfinObject):
