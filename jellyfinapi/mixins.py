...
from jellyfinapi import media, settings, utils
from jellyfinapi.exceptions import BadRequest, NotFound
from jellyfinapi.utils import deprecated, openOrRead


class AdvancedSettingsMixin:
    """ Mixin for Jellyfin objects that can have advanced settings. """

    def preferences(self):
        """ Returns a list of :class:`~jellyfinapi.settings.Preferences` objects. """
        data = self._server.query(self._details_key)
        return self.findItems(data, settings.Preferences, rtag='Preferences')

    def preference(self, pref):
        """ Returns a :class:`~jellyfinapi.settings.Preferences` object for the specified pref.
    ...
    def editAdvanced(self, **kwargs):
        """ Edit a Jellyfin object's advanced settings. """
        data = {}
    ...
    def defaultAdvanced(self):
        """ Edit all of a Jellyfin object's advanced settings to default. """
        data = {}
        key = f'{self.key}/prefs?'
...
class SmartFilterMixin:
    """ Mixin for Jellyfin objects that can have smart filters. """
...
class SplitMergeMixin:
    """ Mixin for Jellyfin objects that can be split and merged. """

    def split(self):
        """ Split duplicated Jellyfin object into separate objects. """
        key = f'{self.key}/split'
        self._server.query(key, method=self._server._session.put)
        return self

    def merge(self, ratingKeys):
        """ Merge other Jellyfin objects into the current object.
...
class UnmatchMatchMixin:
    """ Mixin for Jellyfin objects that can be unmatched and matched. """
    ...
    def matches(self, agent=None, title=None, year=None, language=None):
        """ Return list of (:class:`~jellyfinapi.media.SearchResult`) metadata matches.
            ...
            Examples:
                ...
                1. The default behaviour in Jellyfin Web = no params in jellyfinapi
    ...
    def fixMatch(self, searchResult=None, auto=False, agent=None):
        """ Use match result to update show metadata.
            ...
            Parameters:
                ...
                searchResult (:class:`~jellyfinapi.media.SearchResult`): Search result from
                    ~jellyfinapi.base.matches()
                agent (str): Agent name to be used (imdb, thetvdb, themoviedb, etc.)
        """
        ...
        if auto:
            ...
            raise NotFound('fixMatch() requires either auto=True or '
                           'searchResult=:class:`~jellyfinapi.media.SearchResult`.')
...
class ExtrasMixin:
    """ Mixin for Jellyfin objects that can have extras. """

    def extras(self):
        """ Returns a list of :class:`~jellyfinapi.video.Extra` objects. """
        from jellyfinapi.video import Extra
        data = self._server.query(self._details_key)
        return self.findItems(data, Extra, rtag='Extras')


class HubsMixin:
    """ Mixin for Jellyfin objects that can have related hubs. """

    def hubs(self):
        """ Returns a list of :class:`~jellyfinapi.library.Hub` objects. """
        from jellyfinapi.library import Hub
...
class PlayedUnplayedMixin:
    """ Mixin for Jellyfin objects that can be marked played and unplayed. """
    ...
    def markPlayed(self):
        """ Mark the Jellyfin object as played. """
        key = '/:/scrobble'
        params = {'key': self.ratingKey, 'identifier': 'com.jellyfinapp.plugins.library'}
        self._server.query(key, params=params)
        return self

    def markUnplayed(self):
        """ Mark the Jellyfin object as unplayed. """
        key = '/:/unscrobble'
        params = {'key': self.ratingKey, 'identifier': 'com.jellyfinapp.plugins.library'}
        self._server.query(key, params=params)
        return self
    ...
    def markWatched(self):
        """ Alias to :func:`~jellyfinapi.mixins.PlayedUnplayedMixin.markPlayed`. """
        self.markPlayed()

    def markUnwatched(self):
        """ Alias to :func:`~jellyfinapi.mixins.PlayedUnplayedMixin.markUnplayed`. """
        self.markUnplayed()


class RatingMixin:
    """ Mixin for Jellyfin objects that can have user star ratings. """

    def rate(self, rating=None):
        """ Rate the Jellyfin object. Note: Jellyfin ratings are displayed out of 5 stars (e.g. rating 7.0 = 3.5 stars).

            Parameters:
                rating (float, optional): Rating from 0 to 10. Exclude to reset the rating.

            Raises:
                :exc:`~jellyfinapi.exceptions.BadRequest`: If the rating is invalid.
        """
        if rating is None:
            rating = -1
        elif not isinstance(rating, (int, float)) or rating < 0 or rating > 10:
            raise BadRequest('Rating must be between 0 to 10.')
        key = f'/:/rate?key={self.ratingKey}&identifier=com.jellyfinapp.plugins.library&rating={rating}'
        self._server.query(key, method=self._server._session.put)
        return self


class ArtUrlMixin:
    """ Mixin for Jellyfin objects that can have a background artwork url. """

    @property
    def artUrl(self):
        """ Return the art url for the Jellyfin object. """
        art = self.firstAttr('art', 'grandparentArt')
        return self._server.url(art, includeToken=True) if art else None


class ArtLockMixin:
    """ Mixin for Jellyfin objects that can have a locked background artwork. """

    def lockArt(self):
        """ Lock the background artwork for a Jellyfin object. """
        return self._edit(**{'art.locked': 1})

    def unlockArt(self):
        """ Unlock the background artwork for a Jellyfin object. """
        return self._edit(**{'art.locked': 0})


class ArtMixin(ArtUrlMixin, ArtLockMixin):
    """ Mixin for Jellyfin objects that can have background artwork. """

    def arts(self):
        """ Returns list of available :class:`~jellyfinapi.media.Art` objects. """
    ...
    def setArt(self, art):
        """ Set the background artwork for a Jellyfin object.

            Parameters:
                art (:class:`~jellyfinapi.media.Art`): The art object to select.
        """
        art.select()
        return self
...
class PosterUrlMixin:
    """ Mixin for Jellyfin objects that can have a poster url. """

    @property
    def thumbUrl(self):
        """ Return the thumb url for the Jellyfin object. """
...
class PosterLockMixin:
    """ Mixin for Jellyfin objects that can have a locked poster. """

    def lockPoster(self):
        """ Lock the poster for a Jellyfin object. """
        return self._edit(**{'thumb.locked': 1})

    def unlockPoster(self):
        """ Unlock the poster for a Jellyfin object. """
        return self._edit(**{'thumb.locked': 0})


class PosterMixin(PosterUrlMixin, PosterLockMixin):
    """ Mixin for Jellyfin objects that can have posters. """

    def posters(self):
        """ Returns list of available :class:`~jellyfinapi.media.Poster` objects. """
    ...
    def setPoster(self, poster):
        """ Set the poster for a Jellyfin object.

            Parameters:
                poster (:class:`~jellyfinapi.media.Poster`): The poster object to select.
        """
        poster.select()
        return self
...
class ThemeUrlMixin:
    """ Mixin for Jellyfin objects that can have a theme url. """

    @property
    def themeUrl(self):
        """ Return the theme url for the Jellyfin object. """
...
class ThemeLockMixin:
    """ Mixin for Jellyfin objects that can have a locked theme. """

    def lockTheme(self):
        """ Lock the theme for a Jellyfin object. """
        return self._edit(**{'theme.locked': 1})

    def unlockTheme(self):
        """ Unlock the theme for a Jellyfin object. """
        return self._edit(**{'theme.locked': 0})


class ThemeMixin(ThemeUrlMixin, ThemeLockMixin):
    """ Mixin for Jellyfin objects that can have themes. """

    def themes(self):
        """ Returns list of available :class:`~jellyfinapi.media.Theme` objects. """
    ...
    def uploadTheme(self, url=None, filepath=None, timeout=None):
        """ Upload a theme from url or filepath.
            ...
            Warning: Themes cannot be deleted using JellyfinAPI!
...
class EditFieldMixin:
    """ Mixin for editing Jellyfin object fields. """

    def editField(self, field, value, locked=True, **kwargs):
        """ Edit the field of a Jellyfin object. All field editing methods can be chained together.
            Also see :func:`~jellyfinapi.base.JellyfinPartialObject.batchEdits` for batch editing fields.
...
class AddedAtMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have an added at date. """
...
class ContentRatingMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have a content rating. """
...
class EditionTitleMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have an edition title. """

    def editEditionTitle(self, editionTitle, locked=True):
        """ Edit the edition title. Jellyfin Pass is required to edit this field.
...
class OriginallyAvailableMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have an originally available date. """
...
class OriginalTitleMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have an original title. """
...
class SortTitleMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have a sort title. """
...
class StudioMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have a studio. """
...
class SummaryMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have a summary. """
...
class TaglineMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have a tagline. """
...
class TitleMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have a title. """
...
class TrackArtistMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have a track artist. """
...
class TrackNumberMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have a track number. """
...
class TrackDiscNumberMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have a track disc number. """
...
class PhotoCapturedTimeMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have a captured time. """
...
class UserRatingMixin(EditFieldMixin):
    """ Mixin for Jellyfin objects that can have a user rating. """
...
class EditTagsMixin:
    """ Mixin for editing Jellyfin object tags. """
    ...
    def editTags(self, tag, items, locked=True, remove=False, **kwargs):
        """ Edit the tags of a Jellyfin object. All tag editing methods can be chained together.
            Also see :func:`~jellyfinapi.base.JellyfinPartialObject.batchEdits` for batch editing tags.

            Parameters:
                tag (str): Name of the tag to edit.
                items (List<str> or List<:class:`~jellyfinapi.media.MediaTag`>): List of tags to add or remove.
...
class CollectionMixin(EditTagsMixin):
    """ Mixin for Jellyfin objects that can have collections. """

    def addCollection(self, collections, locked=True):
        """ Add a collection tag(s).

            Parameters:
                collections (List<str> or List<:class:`~jellyfinapi.media.MediaTag`>): List of tags.
                locked (bool): True (default) to lock the field, False to unlock the field.
        """
...
class CountryMixin(EditTagsMixin):
    """ Mixin for Jellyfin objects that can have countries. """

    def addCountry(self, countries, locked=True):
        """ Add a country tag(s).

            Parameters:
                countries (List<str> or List<:class:`~jellyfinapi.media.MediaTag`>): List of tags.
                locked (bool): True (default) to lock the field, False to unlock the field.
        """
...
class DirectorMixin(EditTagsMixin):
    """ Mixin for Jellyfin objects that can have directors. """

    def addDirector(self, directors, locked=True):
        """ Add a director tag(s).

            Parameters:
                directors (List<str> or List<:class:`~jellyfinapi.media.MediaTag`>): List of tags.
                locked (bool): True (default) to lock the field, False to unlock the field.
        """
...
class GenreMixin(EditTagsMixin):
    """ Mixin for Jellyfin objects that can have genres. """

    def addGenre(self, genres, locked=True):
        """ Add a genre tag(s).

            Parameters:
                genres (List<str> or List<:class:`~jellyfinapi.media.MediaTag`>): List of tags.
                locked (bool): True (default) to lock the field, False to unlock the field.
        """
...
class LabelMixin(EditTagsMixin):
    """ Mixin for Jellyfin objects that can have labels. """

    def addLabel(self, labels, locked=True):
        """ Add a label tag(s).

            Parameters:
                labels (List<str> or List<:class:`~jellyfinapi.media.MediaTag`>): List of tags.
                locked (bool): True (default) to lock the field, False to unlock the field.
        """
...
class MoodMixin(EditTagsMixin):
    """ Mixin for Jellyfin objects that can have moods. """

    def addMood(self, moods, locked=True):
        """ Add a mood tag(s).

            Parameters:
                moods (List<str> or List<:class:`~jellyfinapi.media.MediaTag`>): List of tags.
                locked (bool): True (default) to lock the field, False to unlock the field.
        """
...
class ProducerMixin(EditTagsMixin):
    """ Mixin for Jellyfin objects that can have producers. """

    def addProducer(self, producers, locked=True):
        """ Add a producer tag(s).

            Parameters:
                producers (List<str> or List<:class:`~jellyfinapi.media.MediaTag`>): List of tags.
                locked (bool): True (default) to lock the field, False to unlock the field.
        """
...
class SimilarArtistMixin(EditTagsMixin):
    """ Mixin for Jellyfin objects that can have similar artists. """

    def addSimilarArtist(self, artists, locked=True):
        """ Add a similar artist tag(s).

            Parameters:
                artists (List<str> or List<:class:`~jellyfinapi.media.MediaTag`>): List of tags.
                locked (bool): True (default) to lock the field, False to unlock the field.
        """
...
class StyleMixin(EditTagsMixin):
    """ Mixin for Jellyfin objects that can have styles. """

    def addStyle(self, styles, locked=True):
        """ Add a style tag(s).

            Parameters:
                styles (List<str> or List<:class:`~jellyfinapi.media.MediaTag`>): List of tags.
                locked (bool): True (default) to lock the field, False to unlock the field.
        """
...
class TagMixin(EditTagsMixin):
    """ Mixin for Jellyfin objects that can have tags. """

    def addTag(self, tags, locked=True):
        """ Add a tag(s).

            Parameters:
                tags (List<str> or List<:class:`~jellyfinapi.media.MediaTag`>): List of tags.
                locked (bool): True (default) to lock the field, False to unlock the field.
        """
...
class WriterMixin(EditTagsMixin):
    """ Mixin for Jellyfin objects that can have writers. """

    def addWriter(self, writers, locked=True):
        """ Add a writer tag(s).

            Parameters:
                writers (List<str> or List<:class:`~jellyfinapi.media.MediaTag`>): List of tags.
                locked (bool): True (default) to lock the field, False to unlock the field.
        """
...
class WatchlistMixin:
    """ Mixin for Jellyfin objects that can be added to a user's watchlist. """

    def onWatchlist(self, account=None):
        """ Returns True if the item is on the user's watchlist.
            Also see :func:`~jellyfinapi.myjellyfin.MyJellyfinAccount.onWatchlist`.

            Parameters:
                account (:class:`~jellyfinapi.myjellyfin.MyJellyfinAccount`, optional): Account to check item on the watchlist.
                   Note: This is required if you are not connected to a Jellyfin server instance using the admin account.
        """
        try:
            account = account or self._server.myJellyfinAccount()
    ...
    def addToWatchlist(self, account=None):
        """ Add this item to the specified user's watchlist.
            Also see :func:`~jellyfinapi.myjellyfin.MyJellyfinAccount.addToWatchlist`.

            Parameters:
                account (:class:`~jellyfinapi.myjellyfin.MyJellyfinAccount`, optional): Account to add item to the watchlist.
                   Note: This is required if you are not connected to a Jellyfin server instance using the admin account.
        """
        try:
            account = account or self._server.myJellyfinAccount()
    ...
    def removeFromWatchlist(self, account=None):
        """ Remove this item from the specified user's watchlist.
            Also see :func:`~jellyfinapi.myjellyfin.MyJellyfinAccount.removeFromWatchlist`.

            Parameters:
                account (:class:`~jellyfinapi.myjellyfin.MyJellyfinAccount`, optional): Account to remove item from the watchlist.
                   Note: This is required if you are not connected to a Jellyfin server instance using the admin account.
        """
        try:
            account = account or self._server.myJellyfinAccount()
    ...
    def streamingServices(self, account=None):
        """ Return a list of :class:`~jellyfinapi.media.Availability`
            objects for the available streaming services for this item.

            Parameters:
                account (:class:`~jellyfinapi.myjellyfin.MyJellyfinAccount`, optional): Account used to retrieve availability.
                   Note: This is required if you are not connected to a Jellyfin server instance using the admin account.
        """
        try:
            account = account or self._server.myJellyfinAccount()
