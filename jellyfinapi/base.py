...
from jellyfinapi import CONFIG, X_PLEX_CONTAINER_SIZE, log, utils
from jellyfinapi.exceptions import BadRequest, NotFound, UnknownType, Unsupported
...
class JellyfinObject:
    """ Base class for all Jellyfin objects.

        Parameters:
            server (:class:`~jellyfinapi.server.JellyfinServer`): JellyfinServer this client is connected to (optional)
            data (ElementTree): Response from JellyfinServer used to build this object (optional).
            initpath (str): Relative path requested when retrieving specified `data` (optional).
            parent (:class:`~jellyfinapi.base.JellyfinObject`): The parent object that this object is built from (optional).
    """
    TAG = None      # xml element tag
    TYPE = None     # xml element type
    key = None      # jellyfin relative url
    ...
    def __init__(self, server, data, initpath=None, parent=None):
        ...
        self._autoReload = CONFIG.get('jellyfinapi.autoreload', True, bool)
    ...
    def _buildItemOrNone(self, elem, cls=None, initpath=None):
        """ Calls :func:`~jellyfinapi.base.JellyfinObject._buildItem` but returns
        ...
        """ Returns True if this object is a child of the given attributes.
            ...
            Parameters:
                **kwargs (dict): The attributes and values to search for in the parent objects.
                    See all possible `**kwargs*` in :func:`~jellyfinapi.base.JellyfinObject.fetchItem`.
        """
        obj = self
    ...
    def _manuallyLoadXML(self, xml, cls=None):
        """ Manually load an XML string as a :class:`~jellyfinapi.base.JellyfinObject`.

            Parameters:
                xml (str): The XML string to load.
                cls (:class:`~jellyfinapi.base.JellyfinObject`): If you know the class of the
                    items to be fetched, passing this in will help the parser ensure
                    it only returns those items. By default we convert the xml elements
                    with the best guess JellyfinObjects based on tag and type attrs.
        """
    ...
    def fetchItems(self, ekey, cls=None, container_start=None, container_size=None, maxresults=None, **kwargs):
        """ Load the specified key to find and build all items with the specified tag
            ...
            Parameters:
                ekey (str or List<int>): API URL path in Jellyfin to fetch items from. If a list of ints is passed
            ...
            **Using JellyfinAPI Operators**
            ...
            Optionally, JellyfinAPI operators can be specified by *appending* it to the end of the
            attribute for more comjellyfin lookups. For example, passing in ``viewCount__gte=0``
            ...
            * ``__startswith``: Value starts with specified arg.
            ...
            Examples:
                ...
                .. code-block:: python
                    ...
                    fetchItem(ekey, guid__regex=r"com\.jellyfinapp\.agents\.(imdb|themoviedb)://|tt\d+")
        ...
        while True:
            headers['X-Jellyfin-Container-Start'] = str(container_start)
            headers['X-Jellyfin-Container-Size'] = str(container_size)
    ...
    def fetchItem(self, ekey, cls=None, **kwargs):
        """ Load the specified key to find and build the first item with the
            ...
            Parameters:
                ekey (str or int): Path in Jellyfin to fetch items from. If an int is passed
                ...
                etag (str): Only fetch items with the specified tag.
                **kwargs (dict): Optionally add XML attribute to filter the items.
                    See :func:`~jellyfinapi.base.JellyfinObject.fetchItems` for more details
                    on how this is used.
        """
    ...
    def findItems(self, data, cls=None, initpath=None, rtag=None, **kwargs):
        """ Load the specified data to find and build all items with the specified tag
            and attrs. See :func:`~jellyfinapi.base.JellyfinObject.fetchItem` for more details
            on how this is used.
        """
        ...
        """ Reload the data for this object from self.key.
            ...
            Parameters:
                ...
                **kwargs (dict): A dictionary of XML include parameters to exclude or override.
                    All parameters are included by default with the option to override each parameter
                    or disable each parameter individually by setting it to False or 0.
                    See :class:`~jellyfinapi.base.JellyfinPartialObject` for all the available include parameters.
            ...
            Example:
                ...
                .. code-block:: python
                    ...
                    movie = jellyfin.library.section('Movies').get('Cars')

                    # Partial reload of the movie without the `checkFiles` parameter.
                    # Excluding `checkFiles` will prevent the Jellyfin server from reading the
...
class JellyfinPartialObject(JellyfinObject):
    """ Not all objects in the Jellyfin listings return the complete list of elements
    ...
    def __eq__(self, other):
        if isinstance(other, JellyfinPartialObject):
    ...
    def __getattribute__(self, attr):
        # Dragons inside.. :-/
        value = super(JellyfinPartialObject, self).__getattribute__(attr)
        ...
        if isinstance(self, (JellyfinSession, JellyfinHistory)): return value
        ...
        return super(JellyfinPartialObject, self).__getattribute__(attr)

    def analyze(self):
        """ Tell Jellyfin Media Server to performs analysis on it this item to gather
            ...
            * Generate Video Preview Thumbnails: Video preview thumbnails are created,
                ...
                graphical seeking in some Apps. It's also used in the Jellyfin Web App Now
    ...
    def batchEdits(self):
        """ Enable batch editing mode to save API calls.
            Must call :func:`~jellyfinapi.base.JellyfinPartialObject.saveEdits` at the end to save all the edits.
            See :class:`~jellyfinapi.mixins.EditFieldMixin` and :class:`~jellyfinapi.mixins.EditTagsMixin`
            for individual field and tag editing methods.
    ...
    def saveEdits(self):
        """ Save all the batch edits. The object needs to be reloaded manually,
            if required.
            See :func:`~jellyfinapi.base.JellyfinPartialObject.batchEdits` for details.
        """
    ...
    def section(self):
        """ Returns the :class:`~jellyfinapi.library.LibrarySection` this item belongs to. """
        return self._server.library.sectionByID(self.librarySectionID)

    def delete(self):
        """ Delete a media element. This has to be enabled under settings > server > library in jellyfin webui. """
        try:
    ...
    def getWebURL(self, base=None):
        """ Returns the Jellyfin Web URL for a media item.
    ...
    def playQueue(self, *args, **kwargs):
        """ Returns a new :class:`~jellyfinapi.playqueue.PlayQueue` from this media item.
            See :func:`~jellyfinapi.playqueue.PlayQueue.create` for available parameters.
        """
...
class Playable:
    """ This is a general place to store functions specific to media that is Playable.
        ...
        Attributes:
            playlistItemID (int): Playlist item ID (only populated for :class:`~jellyfinapi.playlist.Playlist` items).
            playQueueItemID (int): PlayQueue item ID (only populated for :class:`~jellyfinapi.playlist.PlayQueue` items).
    """
    ...
    def getStreamURL(self, **kwargs):
        """ Returns a stream url that may be used by external applications such as VLC.
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.Unsupported`: When the item doesn't support fetching a stream URL.
        """
        ...
        params = {
            ...
            'X-Jellyfin-Platform': kwargs.pop('platform', 'Chrome')
        }
    ...
    def videoStreams(self):
        """ Returns a list of :class:`~jellyfinapi.media.videoStream` objects for all MediaParts. """
        if self.isPartialObject():
            self.reload()
        return sum((part.videoStreams() for part in self.iterParts()), [])

    def audioStreams(self):
        """ Returns a list of :class:`~jellyfinapi.media.AudioStream` objects for all MediaParts. """
        if self.isPartialObject():
            self.reload()
        return sum((part.audioStreams() for part in self.iterParts()), [])

    def subtitleStreams(self):
        """ Returns a list of :class:`~jellyfinapi.media.SubtitleStream` objects for all MediaParts. """
        if self.isPartialObject():
            self.reload()
        return sum((part.subtitleStreams() for part in self.iterParts()), [])

    def lyricStreams(self):
        """ Returns a list of :class:`~jellyfinapi.media.LyricStream` objects for all MediaParts. """
        if self.isPartialObject():
            self.reload()
    ...
    def play(self, client):
        """ Start playback on the specified client.
            ...
            Parameters:
                client (:class:`~jellyfinapi.client.JellyfinClient`): Client to start playing on.
        """
    ...
    def download(self, savepath=None, keep_original_name=False, **kwargs):
        """ Downloads the media item to the specified location. Returns a list of
            ...
            Parameters:
                ...
                **kwargs (dict): Additional options passed into :func:`~jellyfinapi.audio.Track.getStreamURL`
    ...
    def updateProgress(self, time, state='stopped'):
        """ Set the watched progress for this video.
            ...
            Use :func:`~jellyfinapi.mixins.PlayedUnplayedMixin.markPlayed` or
            :func:`~jellyfinapi.mixins.PlayedUnplayedMixin.markUnplayed` to achieve
            that goal.
        ...
        """
        key = f'/:/progress?key={self.ratingKey}&identifier=com.jellyfinapp.plugins.library&time={time}&state={state}'
    ...
    def updateTimeline(self, time, state='stopped', duration=None):
        ...
        key = (f'/:/timeline?ratingKey={self.ratingKey}&key={self.key}&'
               f'identifier=com.jellyfinapp.plugins.library&time={int(time)}&state={state}{durationStr}')
        self._server.query(key)
        return self


class JellyfinSession(object):
    """ This is a general place to store functions specific to media that is a Jellyfin Session.

        Attributes:
            live (bool): True if this is a live tv session.
            player (:class:`~jellyfinapi.client.JellyfinClient`): JellyfinClient object for the session.
            session (:class:`~jellyfinapi.media.Session`): Session object for the session
                if the session is using bandwidth (None otherwise).
            sessionKey (int): The session key for the session.
            transcodeSession (:class:`~jellyfinapi.media.TranscodeSession`): TranscodeSession object
    ...
    @cached_property
    def user(self):
        """ Returns the :class:`~jellyfinapi.myjellyfin.MyJellyfinAccount` object (for admin)
            or :class:`~jellyfinapi.myjellyfin.MyJellyfinUser` object (for users) for this session.
        """
        myJellyfinAccount = self._server.myJellyfinAccount()
        if self._userId == 1:
            return myJellyfinAccount

        return myJellyfinAccount.user(self._username)
...
class JellyfinHistory(object):
    """ This is a general place to store functions specific to media that is a Jellyfin history item.
...
class MediaContainer(JellyfinObject):
    """ Represents a single MediaContainer.
        ...
        Attributes:
            ...
            librarySectionUUID (str): :class:`~jellyfinapi.library.LibrarySection` UUID.
