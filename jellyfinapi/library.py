...
from jellyfinapi import log, media, utils
from jellyfinapi.base import OPERATORS, JellyfinObject
from jellyfinapi.exceptions import BadRequest, NotFound
...
from jellyfinapi.settings import Setting
from jellyfinapi.utils import deprecated


class Library(JellyfinObject):
    """ Represents a JellyfinServer library. This contains all sections of media defined
        in your Jellyfin server including video, shows and audio.

        Attributes:
            key (str): '/library'
            identifier (str): Unknown ('com.jellyfinapp.plugins.library').
            mediaTagVersion (str): Unknown (/system/bundle/media/flags/)
            server (:class:`~jellyfinapi.server.JellyfinServer`): JellyfinServer this client is connected to.
            title1 (str): 'Jellyfin Library' (not sure how useful this is).
    ...
    def _loadData(self, data):
        ...
        self.mediaTagVersion = ""
        self.title1 = "Jellyfin Library"
        self.title2 = ""
    ...
    def _loadSections(self):
        ...
        for section in self._server.query(key):
            section = libcls.get(section["CollectionType"], LibrarySection)(self._server, section, initpath=key)
            sectionsByID[section.uuid] = section
    ...
    def sections(self):
        """ Returns a list of all media sections in this library. Library sections may be any of
            :class:`~jellyfinapi.library.MovieSection`, :class:`~jellyfinapi.library.ShowSection`,
            :class:`~jellyfinapi.library.MusicSection`, :class:`~jellyfinapi.library.PhotoSection`.
        """
        self._loadSections()
        return list(self._sectionsByID.values())

    def section(self, title):
        """ Returns the :class:`~jellyfinapi.library.LibrarySection` that matches the specified title.
            Note: Multiple library sections with the same title is ambiguous.
            Use :func:`~jellyfinapi.library.Library.sectionByID` instead for an exact match.

            Parameters:
                title (str): Title of the section to return.

            Raises:
                :exc:`~jellyfinapi.exceptions.NotFound`: The library section title is not found on the server.
        """
    ...
    def sectionByID(self, sectionID):
        """ Returns the :class:`~jellyfinapi.library.LibrarySection` that matches the specified sectionID.

            Parameters:
                sectionID (int): ID of the section to return.

            Raises:
                :exc:`~jellyfinapi.exceptions.NotFound`: The library section ID is not found on the server.
        """
    ...
    def hubs(self, sectionID=None, identifier=None, **kwargs):
        """ Returns a list of :class:`~jellyfinapi.library.Hub` across all library sections.
    ...
    def add(self, name='', type='', agent='', scanner='', location='', language='en', *args, **kwargs):
        """ Simplified add for the most common options.
            ...
            Parameters:
                ...
                agent (str): Example com.jellyfinapp.agents.imdb
            ...
            **Photo Preferences**
                ...
                * **agent** (str): com.jellyfinapp.agents.none
                * **enableAutoPhotoTags** (bool): Tag photos. Default value false.
                * **enableBIFGeneration** (bool): Enable video preview thumbnails. Default value true.
                * **includeInGlobal** (bool): Include in dashboard. Default value true.
                * **scanner** (str): Jellyfin Photo Scanner

            **Movie Preferences**

                * **agent** (str): com.jellyfinapp.agents.none, com.jellyfinapp.agents.imdb, tv.jellyfin.agents.movie,
                  com.jellyfinapp.agents.themoviedb
                * **enableBIFGeneration** (bool): Enable video preview thumbnails. Default value true.
                * **enableCinemaTrailers** (bool): Enable Cinema Trailers. Default value true.
                * **includeInGlobal** (bool): Include in dashboard. Default value true.
                * **scanner** (str): Jellyfin Movie, Jellyfin Movie Scanner, Jellyfin Video Files Scanner, Jellyfin Video Files

            **IMDB Movie Options** (com.jellyfinapp.agents.imdb)

                * **title** (bool): Localized titles. Default value false.
                * **extras** (bool): Find trailers and extras automatically (Jellyfin Pass required). Default value true.
                ...
                * **usage** (bool): Send anonymous usage data to Jellyfin. Default value true.

            **TheMovieDB Movie Options** (com.jellyfinapp.agents.themoviedb)
            ...
            **Show Preferences**
                ...
                * **agent** (str): com.jellyfinapp.agents.none, com.jellyfinapp.agents.thetvdb, com.jellyfinapp.agents.themoviedb,
                  tv.jellyfin.agents.series
                * **enableBIFGeneration** (bool): Enable video preview thumbnails. Default value true.
                ...
                * **includeInGlobal** (bool): Include in dashboard. Default value true.
                * **scanner** (str): Jellyfin TV Series, Jellyfin Series Scanner

            **TheTVDB Show Options** (com.jellyfinapp.agents.thetvdb)

                * **extras** (bool): Find trailers and extras automatically (Jellyfin Pass required). Default value true.
                * **native_subs** (bool): Include extras with subtitles in Library language. Default value false.

            **TheMovieDB Show Options** (com.jellyfinapp.agents.themoviedb)
            ...
            **Other Video Preferences**
                ...
                * **agent** (str): com.jellyfinapp.agents.none, com.jellyfinapp.agents.imdb, com.jellyfinapp.agents.themoviedb
                * **enableBIFGeneration** (bool): Enable video preview thumbnails. Default value true.
                * **enableCinemaTrailers** (bool): Enable Cinema Trailers. Default value true.
                * **includeInGlobal** (bool): Include in dashboard. Default value true.
                * **scanner** (str): Jellyfin Movie Scanner, Jellyfin Video Files Scanner

            **IMDB Other Video Options** (com.jellyfinapp.agents.imdb)
            ...
            **TheMovieDB Other Video Options** (com.jellyfinapp.agents.themoviedb)
    ...
    def tags(self, tag):
        """ Returns a list of :class:`~jellyfinapi.library.LibraryMediaTag` objects for the specified tag.

            Parameters:
                tag (str): Tag name (see :data:`~jellyfinapi.utils.TAGTYPES`).
        """
...
class LibrarySection(JellyfinObject):
    """ Base class for a single library section.

        Attributes:
            agent (str): The metadata agent used for the library section (com.jellyfinapp.agents.imdb, etc).
            ...
            scanner (str): Internal scanner used to find media (Jellyfin Movie Scanner, Jellyfin Premium Music Scanner, etc.)
    ...
    def _loadData(self, data):
        ...
        self.agent = None
        self.allowSync = None
        self.art = data["PrimaryImageItemId"]
        self.composite = None
        self.createdAt = None
        self.filters = None
        self.key = None
        self.language = None
        self.locations = None
        self.refreshing = data["RefreshStatus"]
        self.scanner = None
        self.thumb = None
        self.title = data["Name"]
        self.type = data["CollectionType"]
        self.updatedAt = None
        self.uuid = data["ItemId"]
    ...
    def _getTotalDurationStorage(self):
        """ Queries the Jellyfin server for the total library duration and storage and caches the values. """
        data = self._server.query('/media/providers?includeStorage=1')
        xpath = (
            './MediaProvider[@identifier="com.jellyfinapp.plugins.library"]'
    ...
    def totalViewSize(self, libtype=None, includeCollections=True):
        ...
        args = {
            ...
            'X-Jellyfin-Container-Start': 0,
            'X-Jellyfin-Container-Size': 0
        }
    ...
    def delete(self):
        ...
        try:
            ...
            msg += 'You may need to allow this permission in your Jellyfin settings.'
            log.error(msg)
            raise
    ...
    def edit(self, agent=None, **kwargs):
        """ Edit a library. See :class:`~jellyfinapi.library.Library` for example usage.
        ...
        """ Returns the media item with the specified title and kwargs.
            ...
            Parameters:
                ...
                kwargs (dict): Additional search parameters.
                    See :func:`~jellyfinapi.library.LibrarySection.search` for more info.

            Raises:
                :exc:`~jellyfinapi.exceptions.NotFound`: The title is not found in the library.
        """
        try:
    ...
    def getGuid(self, guid):
        """ Returns the media item with the specified external Jellyfin, IMDB, TMDB, or TVDB ID.
            Note: Only available for the Jellyfin Movie and Jellyfin TV Series agents.

            Parameters:
                guid (str): The external guid of the item to return.
                    Examples: Jellyfin ``jellyfin://show/5d9c086c46115600200aa2fe``
                    IMDB ``imdb://tt0944947``, TMDB ``tmdb://1399``, TVDB ``tvdb://121361``.

            Raises:
                :exc:`~jellyfinapi.exceptions.NotFound`: The guid is not found in the library.

            Example:

                .. code-block:: python

                    result1 = library.getGuid('jellyfin://show/5d9c086c46115600200aa2fe')
                    ...
                    result1 = guidLookup['jellyfin://show/5d9c086c46115600200aa2fe']
    ...
    def all(self, libtype=None, **kwargs):
        """ Returns a list of all items from this library section.
            See description of :func:`~jellyfinapi.library.LibrarySection.search()` for details about filtering / sorting.
        """
        libtype = libtype or self.TYPE
        return self.search(libtype=libtype, **kwargs)

    def folders(self):
        """ Returns a list of available :class:`~jellyfinapi.library.Folder` for this library section.
        """
        key = f'/library/sections/{self.key}/folder'
        return self.fetchItems(key, Folder)

    def managedHubs(self):
        """ Returns a list of available :class:`~jellyfinapi.library.ManagedHub` for this library section.
        """
    ...
    def hubs(self):
        """ Returns a list of available :class:`~jellyfinapi.library.Hub` for this library section.
        """
        key = f'/hubs/sections/{self.key}?includeStations=1'
        return self.fetchItems(key)

    def agents(self):
        """ Returns a list of available :class:`~jellyfinapi.media.Agent` for this library section.
        """
    ...
    def analyze(self):
        """ Run an analysis on all of the items in this library section. See
            See :func:`~jellyfinapi.base.JellyfinPartialObject.analyze` for more details.
        """
    ...
    def _loadFilters(self):
        """ Retrieves and caches the list of :class:`~jellyfinapi.library.FilteringType` and
            list of :class:`~jellyfinapi.library.FilteringFieldType` for this library section.
        """
    ...
    def filterTypes(self):
        """ Returns a list of available :class:`~jellyfinapi.library.FilteringType` for this library section. """
        if self._filterTypes is None:
            self._loadFilters()
        return self._filterTypes

    def getFilterType(self, libtype=None):
        """ Returns a :class:`~jellyfinapi.library.FilteringType` for a specified libtype.
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.NotFound`: Unknown libtype for this library.
        """
        return None
        # libtype = libtype or self.TYPE
        # try:
    ...
    def fieldTypes(self):
        """ Returns a list of available :class:`~jellyfinapi.library.FilteringFieldType` for this library section. """
        if self._fieldTypes is None:
            self._loadFilters()
        return self._fieldTypes

    def getFieldType(self, fieldType):
        """ Returns a :class:`~jellyfinapi.library.FilteringFieldType` for a specified fieldType.
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.NotFound`: Unknown fieldType for this library.
        """
        try:
    ...
    def listFilters(self, libtype=None):
        """ Returns a list of available :class:`~jellyfinapi.library.FilteringFilter` for a specified libtype.
    ...
    def listSorts(self, libtype=None):
        """ Returns a list of available :class:`~jellyfinapi.library.FilteringSort` for a specified libtype.
    ...
    def listFields(self, libtype=None):
        """ Returns a list of available :class:`~jellyfinapi.library.FilteringFields` for a specified libtype.
    ...
    def listOperators(self, fieldType):
        """ Returns a list of available :class:`~jellyfinapi.library.FilteringOperator` for a specified fieldType.
    ...
    def listFilterChoices(self, field, libtype=None):
        """ Returns a list of available :class:`~jellyfinapi.library.FilterChoice` for a specified
            :class:`~jellyfinapi.library.FilteringFilter` or filter field.
            This is the list of available values for a custom filter
            (`screenshot <../_static/images/LibrarySection.search.png>`__).

            Parameters:
                field (str): :class:`~jellyfinapi.library.FilteringFilter` object,
                    or the name of the field (genre, year, contentRating, etc.).
                libtype (str, optional): The library type to filter (movie, show, season, episode,
                    artist, album, track, photoalbum, photo, collection).

            Raises:
                :exc:`~jellyfinapi.exceptions.BadRequest`: Invalid filter field.
                :exc:`~jellyfinapi.exceptions.NotFound`: Unknown filter field.
    ...
    def _validateFieldValueTag(self, value, filterField, libtype):
        """ Validates a filter tag value. A filter tag value can be a :class:`~jellyfinapi.library.FilterChoice` object,
            a :class:`~jellyfinapi.media.MediaTag` object, the exact name :attr:`MediaTag.tag` (*str*),
    ...
    def _validateSortFields(self, sort, libtype=None):
        """ Validates a list of filter sort fields is available for the library. Sort fields can be a
            list of :class:`~jellyfinapi.library.FilteringSort` objects, or a comma separated string.
    ...
    def _validateSortField(self, sort, libtype=None):
        """ Validates a filter sort field is available for the library. A sort field can be a
            :class:`~jellyfinapi.library.FilteringSort` object, or a string.
    ...
    def hubSearch(self, query, mediatype=None, limit=None):
        """ Returns the hub search results for this library. See :func:`jellyfinapi.server.JellyfinServer.search`
    ...
    def search(self, title=None, sort=None, maxresults=None, libtype=None,
        ...
        """ Search the library. The http requests will be batched in container_size. If you are only looking for the
            ...
            Parameters:
                ...
                sort (:class:`~jellyfinapi.library.FilteringSort` or str or list, optional): A field to sort the results.
                ...
                libtype (str, optional): Return results of a specific type (movie, show, season, episode,
                    ...
                    return :class:`~jellyfinapi.video.Episode` objects)
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.BadRequest`: When the sort or filter is invalid.
                :exc:`~jellyfinapi.exceptions.NotFound`: When applying an unknown sort or filter.

            **Sorting Results**

            The search results can be sorted by including the ``sort`` parameter.

            * See :func:`~jellyfinapi.library.LibrarySection.listSorts` to get a list of available sort fields.

            The ``sort`` parameter can be a :class:`~jellyfinapi.library.FilteringSort` object or a sort string in the
            format ``field:dir``. The sort direction ``dir`` can be ``asc``, ``desc``, or ``nullsLast``. Omitting the
            sort direction or using a :class:`~jellyfinapi.library.FilteringSort` object will sort the results in the default
            direction of the field. Multi-sorting on multiple fields can be achieved by using a comma separated list of
            sort strings, or a list of :class:`~jellyfinapi.library.FilteringSort` object or strings.
            ...
            **Using Jellyfin Filters**

            Any of the available custom filters can be applied to the search results
            (`screenshot <../_static/images/LibrarySection.search.png>`__).

            * See :func:`~jellyfinapi.library.LibrarySection.listFields` to get a list of all available fields.
            * See :func:`~jellyfinapi.library.LibrarySection.listOperators` to get a list of all available operators.
            * See :func:`~jellyfinapi.library.LibrarySection.listFilterChoices` to get a list of all available filter values.
            ...
            * **actor** (:class:`~jellyfinapi.media.MediaTag`): Search for the name of an actor.
            ...
            * **collection** (:class:`~jellyfinapi.media.MediaTag`): Search for the name of a collection.
            * **contentRating** (:class:`~jellyfinapi.media.MediaTag`): Search for a specific content rating.
            * **country** (:class:`~jellyfinapi.media.MediaTag`): Search for the name of a country.
            * **decade** (*int*): Search for a specific decade (e.g. 2000).
            * **director** (:class:`~jellyfinapi.media.MediaTag`): Search for the name of a director.
            * **duplicate** (*bool*) Search for duplicate items.
            * **genre** (:class:`~jellyfinapi.media.MediaTag`): Search for a specific genre.
            * **hdr** (*bool*): Search for HDR items.
            * **inProgress** (*bool*): Search for in progress items.
            * **label** (:class:`~jellyfinapi.media.MediaTag`): Search for a specific label.
            * **lastViewedAt** (*datetime*): Search for items watched before or after a date. See operators below.
            * **mood** (:class:`~jellyfinapi.media.MediaTag`): Search for a specific mood.
            * **producer** (:class:`~jellyfinapi.media.MediaTag`): Search for the name of a producer.
            * **resolution** (*str*): Search for a specific resolution (e.g. 1080).
            * **studio** (*str*): Search for the name of a studio.
            * **style** (:class:`~jellyfinapi.media.MediaTag`): Search for a specific style.
            ...
            * **writer** (:class:`~jellyfinapi.media.MediaTag`): Search for the name of a writer.
            * **year** (*int*): Search for a specific year.

            Tag type filter values can be a :class:`~jellyfinapi.library.FilterChoice` object,
            :class:`~jellyfinapi.media.MediaTag` object, the exact name :attr:`MediaTag.tag` (*str*),
            or the exact id :attr:`MediaTag.id` (*int*).
            ...
            **Using Jellyfin Operators**
            ...
            Type: :class:`~jellyfinapi.media.MediaTag` or *subtitleLanguage* or *audioLanguage*
            ...
            **Using Advanced Filters**
            ...
            Any of the Jellyfin filters described above can be combined into a single ``filters`` dictionary that mimics
            the advanced filters used in Jellyfin Web with a tree of ``and``/``or`` branches. Each level of the tree must
            ...
            **Using JellyfinAPI Operators**

            For even more advanced filtering which cannot be achieved in Jellyfin, the JellyfinAPI operators can be applied
            to any XML attribute. See :func:`jellyfinapi.base.JellyfinObject.fetchItems` for a list of operators and how they
            are used. Note that using the Jellyfin filters above will be faster since the filters are applied by the Jellyfin
            server before the results are returned to JellyfinAPI. Using the JellyfinAPI operators requires the Jellyfin server
            to return *all* results to allow JellyfinAPI to do the filtering. The Jellyfin filters and the JellyfinAPI operators
    ...
    def _locations(self):
        """ Returns a list of :class:`~jellyfinapi.library.Location` objects
        """
    ...
    def sync(self, policy, mediaSettings, client=None, clientId=None, title=None, sort=None, libtype=None,
        ...
        """ Add current library section as sync item for specified device.
            See description of :func:`~jellyfinapi.library.LibrarySection.search` for details about filtering / sorting
            and :func:`~jellyfinapi.myjellyfin.MyJellyfinAccount.sync` for possible exceptions.

            Parameters:
                policy (:class:`~jellyfinapi.sync.Policy`): policy of syncing the media (how many items to sync and process
                                                       watched media or not), generated automatically when method
                                                       called on specific LibrarySection object.
                mediaSettings (:class:`~jellyfinapi.sync.MediaSettings`): Transcoding settings used for the media, generated
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.BadRequest`: When the library is not allowed to sync.
                :exc:`~jellyfinapi.exceptions.BadRequest`: When the sort or filter is invalid.
                :exc:`~jellyfinapi.exceptions.NotFound`: When applying an unknown sort or filter.
            ...
            Example:
                ...
                .. code-block:: python
                    ...
                    from jellyfinapi import myjellyfin
                    from jellyfinapi.sync import Policy, MediaSettings, VIDEO_QUALITY_3_MBPS_720p

                    c = myjellyfin.MyJellyfinAccount()
                    target = c.device('Jellyfin Client')
        ...
        """
        from jellyfinapi.sync import SyncItem
        ...
        return myjellyfin.sync(client=client, clientId=clientId, sync_item=sync_item)
    ...
    def createCollection(self, title, items=None, smart=False, limit=None,
        ...
        """ Alias for :func:`~jellyfinapi.server.JellyfinServer.createCollection` using this
            :class:`~jellyfinapi.library.LibrarySection`.
        """
    ...
    def collection(self, title):
        """ Returns the collection with the specified title.
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.NotFound`: Unable to find collection.
        """
        try:
    ...
    def createPlaylist(self, title, items=None, smart=False, limit=None,
        ...
        """ Alias for :func:`~jellyfinapi.server.JellyfinServer.createPlaylist` using this
            :class:`~jellyfinapi.library.LibrarySection`.
        """
    ...
    def playlist(self, title):
        """ Returns the playlist with the specified title.
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.NotFound`: Unable to find playlist.
        """
        try:
    ...
    def getWebURL(self, base=None, tab=None, key=None):
        """ Returns the Jellyfin Web URL for the library.

            Parameters:
                base (str): The base URL before the fragment (``#!``).
                    Default is https://app.jellyfin.tv/desktop.
    ...
    def common(self, items):
        """ Returns a :class:`~jellyfinapi.library.Common` object for the specified items. """
        params = {
    ...
    def multiEdit(self, items, **kwargs):
        """ Edit multiple objects at once.
            ...
            See :class:`~jellyfinapi.LibrarySection.batchMultiEdits` instead.

            Parameters:
                items (List): List of :class:`~jellyfinapi.audio.Audio`, :class:`~jellyfinapi.video.Video`,
                    :class:`~jellyfinapi.photo.Photo`, or :class:`~jellyfinapi.collection.Collection`
    ...
    def batchMultiEdits(self, items):
        """ Enable batch multi-editing mode to save API calls.
            Must call :func:`~jellyfinapi.library.LibrarySection.saveMultiEdits` at the end to save all the edits.
            See :class:`~jellyfinapi.mixins.EditFieldMixin` and :class:`~jellyfinapi.mixins.EditTagsMixin`
    ...
    def saveMultiEdits(self):
        """ Save all the batch multi-edits.
            See :func:`~jellyfinapi.library.LibrarySection.batchMultiEdits` for details.
        """
        if not isinstance(self._edits, dict):
...
class MovieSection(LibrarySection, MovieEditMixins):
    """ Represents a :class:`~jellyfinapi.library.LibrarySection` section containing movies.
    ...
    def searchMovies(self, **kwargs):
        """ Search for a movie. See :func:`~jellyfinapi.library.LibrarySection.search` for usage. """
    ...
    def sync(self, videoQuality, limit=None, unwatched=False, **kwargs):
        """ Add current Movie library section as sync item for specified device.
            See description of :func:`~jellyfinapi.library.LibrarySection.search` for details about filtering / sorting and
            :func:`~jellyfinapi.library.LibrarySection.sync` for details on syncing libraries and possible exceptions.

            Parameters:
                videoQuality (int): idx of quality of the video, one of VIDEO_QUALITY_* values defined in
                                    :mod:`~jellyfinapi.sync` module.
            ...
            Example:
                ...
                .. code-block:: python
                    ...
                    from jellyfinapi import myjellyfin
                    from jellyfinapi.sync import VIDEO_QUALITY_3_MBPS_720p
        ...
        """
        from jellyfinapi.sync import Policy, MediaSettings
...
class ShowSection(LibrarySection, ShowEditMixins, SeasonEditMixins, EpisodeEditMixins):
    """ Represents a :class:`~jellyfinapi.library.LibrarySection` section containing tv shows.
    ...
    def searchShows(self, **kwargs):
        """ Search for a show. See :func:`~jellyfinapi.library.LibrarySection.search` for usage. """
        return self.search(libtype='show', **kwargs)

    def searchSeasons(self, **kwargs):
        """ Search for a season. See :func:`~jellyfinapi.library.LibrarySection.search` for usage. """
        return self.search(libtype='season', **kwargs)

    def searchEpisodes(self, **kwargs):
        """ Search for an episode. See :func:`~jellyfinapi.library.LibrarySection.search` for usage. """
...
class MusicSection(LibrarySection, ArtistEditMixins, AlbumEditMixins, TrackEditMixins):
    """ Represents a :class:`~jellyfinapi.library.LibrarySection` section containing music artists.
    ...
    def albums(self):
        """ Returns a list of :class:`~jellyfinapi.audio.Album` objects in this section. """
        key = f'/library/sections/{self.key}/albums'
        return self.fetchItems(key)

    def stations(self):
        """ Returns a list of :class:`~jellyfinapi.playlist.Playlist` stations in this section. """
        return next((hub.items for hub in self.hubs() if hub.context == 'hub.music.stations'), None)

    def searchArtists(self, **kwargs):
        """ Search for an artist. See :func:`~jellyfinapi.library.LibrarySection.search` for usage. """
        return self.search(libtype='artist', **kwargs)

    def searchAlbums(self, **kwargs):
        """ Search for an album. See :func:`~jellyfinapi.library.LibrarySection.search` for usage. """
        return self.search(libtype='album', **kwargs)

    def searchTracks(self, **kwargs):
        """ Search for a track. See :func:`~jellyfinapi.library.LibrarySection.search` for usage. """
        ...
        """ Add current Music library section as sync item for specified device.
            ...
            Example:
                ...
                .. code-block:: python
                    ...
                    from jellyfinapi import myjellyfin
                    from jellyfinapi.sync import AUDIO_BITRATE_320_KBPS
...
class PhotoSection(LibrarySection, PhotoalbumEditMixins, PhotoEditMixins):
    """ Represents a :class:`~jellyfinapi.library.LibrarySection` section containing photos.
    ...
    def searchAlbums(self, **kwargs):
        """ Search for a photo album. See :func:`~jellyfinapi.library.LibrarySection.search` for usage. """
        return self.search(libtype='photoalbum', **kwargs)

    def searchPhotos(self, **kwargs):
        """ Search for a photo. See :func:`~jellyfinapi.library.LibrarySection.search` for usage. """
        ...
        """ Add current Music library section as sync item for specified device.
            ...
            Example:
                ...
                .. code-block:: python
                    ...
                    from jellyfinapi import myjellyfin
                    from jellyfinapi.sync import PHOTO_QUALITY_HIGH
...
class LibraryTimeline(JellyfinObject):
    """Represents a LibrarySection timeline.
        ...
        Attributes:
            ...
            identifier (str): "com.jellyfinapp.plugins.library"
...
@utils.registerJellyfinObject
class Hub(JellyfinObject):
    """ Represents a single Hub (or category) in the JellyfinServer search.
    ...
    def section(self):
        """ Returns the :class:`~jellyfinapi.library.LibrarySection` this hub belongs to.
        """
...
class LibraryMediaTag(JellyfinObject):
    """ Base class of library media tags.
        ...
        Attributes:
            ...
            tagKey (str): The Jellyfin Discover ratingKey (guid) for people.
...
class FilteringType(JellyfinObject):
    """ Represents a single filtering Type object for a library.

        Attributes:
            TAG (str): 'Type'
            active (bool): True if this filter type is currently active.
            fields (List<:class:`~jellyfinapi.library.FilteringField`>): List of field objects.
            filters (List<:class:`~jellyfinapi.library.FilteringFilter`>): List of filter objects.
            key (str): The API URL path for the libtype filter.
            sorts (List<:class:`~jellyfinapi.library.FilteringSort`>): List of sort objects.
    ...
    def _loadData(self, data):
        ...
        # but not exposed on the Jellyfin server
...
class FilteringFilter(JellyfinObject):
    """ Represents a single Filter object for a :class:`~jellyfinapi.library.FilteringType`.

        Attributes:
            TAG (str): 'Filter'
            filter (str): The key for the filter.
            filterType (str): The :class:`~jellyfinapi.library.FilteringFieldType` type (string, boolean, integer, date, etc).
...
class FilteringSort(JellyfinObject):
    """ Represents a single Sort object for a :class:`~jellyfinapi.library.FilteringType`.
...
class FilteringField(JellyfinObject):
    """ Represents a single Field object for a :class:`~jellyfinapi.library.FilteringType`.

        Attributes:
            TAG (str): 'Field'
            key (str): The URL key for the filter field.
            title (str): The title of the filter field.
            type (str): The :class:`~jellyfinapi.library.FilteringFieldType` type (string, boolean, integer, date, etc).
...
class FilteringFieldType(JellyfinObject):
    """ Represents a single FieldType for library filtering.

        Attributes:
            TAG (str): 'FieldType'
            type (str): The filtering data type (string, boolean, integer, date, etc).
            operators (List<:class:`~jellyfinapi.library.FilteringOperator`>): List of operator objects.
    """
    TAG = 'FieldType'
...
class FilteringOperator(JellyfinObject):
    """ Represents an single Operator for a :class:`~jellyfinapi.library.FilteringFieldType`.
...
class FilterChoice(JellyfinObject):
    """ Represents a single FilterChoice object.
        ...
        object returned in the result set of :func:`~jellyfinapi.library.LibrarySection.listFilterChoices`.
...
class ManagedHub(JellyfinObject):
    ...
    def move(self, after=None):
        """ Move a managed hub to a new position in the library's Managed Recommendations.
            ...
            Parameters:
                after (obj): :class:`~jellyfinapi.library.ManagedHub` object to move the item after in the collection.

            Raises:
                :class:`jellyfinapi.exceptions.BadRequest`: When trying to move a Hub that is not a Managed Recommendation.
        """
    ...
    def remove(self):
        """ Removes a managed hub from the library's Managed Recommendations.
            ...
            Raises:
                :class:`jellyfinapi.exceptions.BadRequest`: When trying to remove a Hub that is not a Managed Recommendation
...
class Folder(JellyfinObject):
    ...
    def subfolders(self):
        """ Returns a list of available :class:`~jellyfinapi.library.Folder` for this folder.
    ...
    def allSubfolders(self):
        """ Returns a list of all available :class:`~jellyfinapi.library.Folder` for this folder.
            Only returns :class:`~jellyfinapi.library.Folder`.
        """
        folders = []
...
class FirstCharacter(JellyfinObject):
...
class Path(JellyfinObject):
    ...
    def browse(self, includeFiles=True):
        """ Alias for :func:`~jellyfinapi.server.JellyfinServer.browse`. """
        return self._server.browse(self, includeFiles)

    def walk(self):
        """ Alias for :func:`~jellyfinapi.server.JellyfinServer.walk`. """
        for path, paths, files in self._server.walk(self):
            yield path, paths, files


@utils.registerJellyfinObject
class File(JellyfinObject):
...
@utils.registerJellyfinObject
class Common(JellyfinObject):
    """ Represents a Common element from a library. This object lists common fields between multiple objects.

        Attributes:
            TAG (str): 'Common'
            collections (List<:class:`~jellyfinapi.media.Collection`>): List of collection objects.
            contentRating (str): Content rating of the items.
            countries (List<:class:`~jellyfinapi.media.Country`>): List of countries objects.
            directors (List<:class:`~jellyfinapi.media.Director`>): List of director objects.
            editionTitle (str): Edition title of the items.
            fields (List<:class:`~jellyfinapi.media.Field`>): List of field objects.
            genres (List<:class:`~jellyfinapi.media.Genre`>): List of genre objects.
            grandparentRatingKey (int): Grandparent rating key of the items.
            grandparentTitle (str): Grandparent title of the items.
            guid (str): Jellyfin GUID of the items.
            guids (List<:class:`~jellyfinapi.media.Guid`>): List of guid objects.
            index (int): Index of the items.
            key (str): API URL (/library/metadata/<ratingkey>).
            labels (List<:class:`~jellyfinapi.media.Label`>): List of label objects.
            mixedFields (List<str>): List of mixed fields.
            moods (List<:class:`~jellyfinapi.media.Mood`>): List of mood objects.
            ...
            producers (List<:class:`~jellyfinapi.media.Producer`>): List of producer objects.
            ratingKey (int): Rating key of the items.
            ratings (List<:class:`~jellyfinapi.media.Rating`>): List of rating objects.
            roles (List<:class:`~jellyfinapi.media.Role`>): List of role objects.
            studio (str): Studio name of the items.
            styles (List<:class:`~jellyfinapi.media.Style`>): List of style objects.
            ...
            writers (List<:class:`~jellyfinapi.media.Writer`>): List of writer objects.
