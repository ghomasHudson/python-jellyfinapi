...
from jellyfinapi.base import Playable, JellyfinPartialObject
from jellyfinapi.exceptions import BadRequest, NotFound, Unsupported
from jellyfinapi.library import LibrarySection, MusicSection
from jellyfinapi.mixins import SmartFilterMixin, ArtMixin, PosterMixin, PlaylistEditMixins
...
class Playlist(
    ...
    """ Represents a single Playlist.
        ...
        Attributes:
            ...
            guid (str): Jellyfin GUID for the playlist (com.jellyfinapp.agents.none://XXXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXX).
    ...
    def filters(self):
        """ Returns the search filter dict for smart playlist.
            The filter dict be passed back into :func:`~jellyfinapi.library.LibrarySection.search`
    ...
    def section(self):
        """ Returns the :class:`~jellyfinapi.library.LibrarySection` this smart playlist belongs to.

            Raises:
                :class:`jellyfinapi.exceptions.BadRequest`: When trying to get the section for a regular playlist.
                :class:`jellyfinapi.exceptions.Unsupported`: When unable to determine the library section.
        """
        if not self.smart:
    ...
    def item(self, title):
        """ Returns the item in the playlist that matches the specified title.
            ...
            Raises:
                :class:`jellyfinapi.exceptions.NotFound`: When the item is not found in the playlist.
        """
        for item in self.items():
    ...
    def get(self, title):
        """ Alias to :func:`~jellyfinapi.playlist.Playlist.item`. """
        ...
        """ Add items to the playlist.
            ...
            Parameters:
                items (List): List of :class:`~jellyfinapi.audio.Audio`, :class:`~jellyfinapi.video.Video`,
                    or :class:`~jellyfinapi.photo.Photo` objects to be added to the playlist.

            Raises:
                :class:`jellyfinapi.exceptions.BadRequest`: When trying to add items to a smart playlist.
        """
        if self.smart:
    ...
    def removeItems(self, items):
        """ Remove items from the playlist.
            ...
            Parameters:
                items (List): List of :class:`~jellyfinapi.audio.Audio`, :class:`~jellyfinapi.video.Video`,
                    or :class:`~jellyfinapi.photo.Photo` objects to be removed from the playlist.

            Raises:
                :class:`jellyfinapi.exceptions.BadRequest`: When trying to remove items from a smart playlist.
                :class:`jellyfinapi.exceptions.NotFound`: When the item does not exist in the playlist.
        """
        if self.smart:
    ...
    def moveItem(self, item, after=None):
        """ Move an item to a new position in the playlist.
            ...
            Parameters:
                items (obj): :class:`~jellyfinapi.audio.Audio`, :class:`~jellyfinapi.video.Video`,
                    or :class:`~jellyfinapi.photo.Photo` objects to be moved in the playlist.
                after (obj): :class:`~jellyfinapi.audio.Audio`, :class:`~jellyfinapi.video.Video`,
                    or :class:`~jellyfinapi.photo.Photo` objects to move the item after in the playlist.

            Raises:
                :class:`jellyfinapi.exceptions.BadRequest`: When trying to move items in a smart playlist.
                :class:`jellyfinapi.exceptions.NotFound`: When the item or item after does not exist in the playlist.
        """
        if self.smart:
    ...
    def updateFilters(self, limit=None, sort=None, filters=None, **kwargs):
        """ Update the filters for a smart playlist.
            ...
            Raises:
                :class:`jellyfinapi.exceptions.BadRequest`: When trying update filters for a regular playlist.
        """
        if not self.smart:
    ...
    def create(cls, server, title, section=None, items=None, smart=False, limit=None,
        ...
        """ Create a playlist.
            ...
            Parameters:
                server (:class:`~jellyfinapi.server.JellyfinServer`): Server to create the playlist on.
        ...
        """ Add the playlist as a sync item for the specified device.
            ...
            Parameters:
                videoQuality (int): idx of quality of the video, one of VIDEO_QUALITY_* values defined in
                                    :mod:`~jellyfinapi.sync` module. Used only when playlist contains video.
                photoResolution (str): maximum allowed resolution for synchronized photos, see PHOTO_QUALITY_* values in
                                       the module :mod:`~jellyfinapi.sync`. Used only when playlist contains photos.
                audioBitrate (int): maximum bitrate for synchronized music, better use one of MUSIC_BITRATE_* values
                                    from the module :mod:`~jellyfinapi.sync`. Used only when playlist contains audio.
            ...
            Raises:
                :exc:`~jellyfinapi.exceptions.BadRequest`: When playlist is not allowed to sync.
                :exc:`~jellyfinapi.exceptions.Unsupported`: When playlist content is unsupported.

            Returns:
                :class:`~jellyfinapi.sync.SyncItem`: A new instance of the created sync item.
        """
