...
from jellyfinapi import log, utils
from jellyfinapi.base import JellyfinObject
from jellyfinapi.exceptions import BadRequest, NotFound


class Settings(JellyfinObject):
    """ Container class for all settings. Allows getting and setting JellyfinServer settings.
    ...
    def all(self):
        """ Returns a list of all :class:`~jellyfinapi.settings.Setting` objects available. """
        return [v for id, v in sorted(self._settings.items())]

    def get(self, id):
        """ Return the :class:`~jellyfinapi.settings.Setting` object with the specified id. """
    ...
    def groups(self):
        """ Returns a dict of lists for all :class:`~jellyfinapi.settings.Setting`
    ...
    def group(self, group):
        """ Return a list of all :class:`~jellyfinapi.settings.Setting` objects in the specified group.
    ...
    def save(self):
        """ Save any outstanding setting changes to the :class:`~jellyfinapi.server.JellyfinServer`. This
            performs a full reload() of Settings after complete.
        """
        params = {}
        for setting in self.all():
            if setting._setValue:
                log.info('Saving JellyfinServer setting %s = %s', setting.id, setting._setValue)
...
class Setting(JellyfinObject):
    """ Represents a single Jellyfin setting.
    ...
    def set(self, value):
        """ Set a new value for this setting. NOTE: You must call jellyfin.settings.save() for before
            any changes to setting values are persisted to the :class:`~jellyfinapi.server.JellyfinServer`.
        """
