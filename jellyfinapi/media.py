...
from jellyfinapi import log, settings, utils
...
class Media(JellyfinObject):
    """ Container object for all MediaPart objects. Provides useful data about the
        ...
        Attributes:
            ...
            parts (List<:class:`~jellyfinapi.media.MediaPart`>): List of media part objects.
    ...
    def isOptimizedVersion(self):
        """ Returns True if the media is a Jellyfin optimized version. """
...
@utils.registerJellyfinObject
class MediaPart(JellyfinObject):
    """ Represents a single media part (often a single file) for the media this belongs to.
        ...
        Attributes:
            ...
            deepAnalysisVersion (int): The Jellyfin deep analysis version for the file.
            ...
            streams (List<:class:`~jellyfinapi.media.MediaPartStream`>): List of stream objects.
    ...
    def _buildStreams(self, data):
        """ Returns a list of :class:`~jellyfinapi.media.MediaPartStream` objects in this MediaPart. """
    ...
    def videoStreams(self):
        """ Returns a list of :class:`~jellyfinapi.media.VideoStream` objects in this MediaPart. """
        return [stream for stream in self.streams if isinstance(stream, VideoStream)]

    def audioStreams(self):
        """ Returns a list of :class:`~jellyfinapi.media.AudioStream` objects in this MediaPart. """
        return [stream for stream in self.streams if isinstance(stream, AudioStream)]

    def subtitleStreams(self):
        """ Returns a list of :class:`~jellyfinapi.media.SubtitleStream` objects in this MediaPart. """
        return [stream for stream in self.streams if isinstance(stream, SubtitleStream)]

    def lyricStreams(self):
        """ Returns a list of :class:`~jellyfinapi.media.LyricStream` objects in this MediaPart. """
        return [stream for stream in self.streams if isinstance(stream, LyricStream)]

    def setSelectedAudioStream(self, stream):
        """ Set the selected :class:`~jellyfinapi.media.AudioStream` for this MediaPart.

            Parameters:
                stream (:class:`~jellyfinapi.media.AudioStream`): Audio stream to set as selected
        """
    ...
    def setSelectedSubtitleStream(self, stream):
        """ Set the selected :class:`~jellyfinapi.media.SubtitleStream` for this MediaPart.

            Parameters:
                stream (:class:`~jellyfinapi.media.SubtitleStream`): Subtitle stream to set as selected.
        """
        key = f'/library/parts/{self.id}'
        params = {'allParts': 1}
...
class MediaPartStream(JellyfinObject):
    """ Base class for media streams. These consist of video, audio, subtitles, and lyrics.
        ...
        Attributes:
            ...
            streamType (int): The stream type (1= :class:`~jellyfinapi.media.VideoStream`,
                2= :class:`~jellyfinapi.media.AudioStream`, 3= :class:`~jellyfinapi.media.SubtitleStream`).
...
class VideoStream(MediaPartStream):
    """ Represents a video stream within a :class:`~jellyfinapi.media.MediaPart`.
...
class AudioStream(MediaPartStream):
    """ Represents a audio stream within a :class:`~jellyfinapi.media.MediaPart`.
    ...
    def setSelected(self):
        """ Sets this audio stream as the selected audio stream.
            Alias for :func:`~jellyfinapi.media.MediaPart.setSelectedAudioStream`.
        """
...
class SubtitleStream(MediaPartStream):
    ...
    def setSelected(self):
        """ Sets this subtitle stream as the selected subtitle stream.
            Alias for :func:`~jellyfinapi.media.MediaPart.setSelectedSubtitleStream`.
        """
...
class LyricStream(MediaPartStream):
    """ Represents a lyric stream within a :class:`~jellyfinapi.media.MediaPart`.
        ...
        Attributes:
            ...
            provider (str): The provider of the lyric stream (ex: com.jellyfinapp.agents.lyricfind).
...
@utils.registerJellyfinObject
class Session(JellyfinObject):
    """ Represents a current session.

        Attributes:
            TAG (str): 'Session'
            id (str): The unique identifier for the session.
            bandwidth (int): The Jellyfin streaming brain reserved bandwidth for the session.
...
@utils.registerJellyfinObject
class TranscodeSession(JellyfinObject):
...
@utils.registerJellyfinObject
class TranscodeJob(JellyfinObject):
...
@utils.registerJellyfinObject
class Optimized(JellyfinObject):
    ...
    def items(self):
        """ Returns a list of all :class:`~jellyfinapi.media.Video` objects
...
@utils.registerJellyfinObject
class Conversion(JellyfinObject):
...
class MediaTag(JellyfinObject):
    """ Base class for media tags used for filtering and searching your library
        ...
        Attributes:
            ...
            role (str): The name of the character role for :class:`~jellyfinapi.media.Role` only.
            ...
            tagKey (str): Jellyfin GUID for the actor/actress for :class:`~jellyfinapi.media.Role` only.
            thumb (str): URL to thumbnail image for :class:`~jellyfinapi.media.Role` only.
    """
...
class Collection(MediaTag):
    ...
    def collection(self):
        """ Return the :class:`~jellyfinapi.collection.Collection` object for this collection tag.
        """
...
class BaseResource(JellyfinObject):
    ...
    def resourceFilepath(self):
        """ Returns the file path to the resource in the Jellyfin Media Server data directory.
...
class Marker(JellyfinObject):
    """ Represents a single Marker media tag.
        ...
        Attributes:
            ...
            version (int): The Jellyfin marker version.
    """
    TAG = 'Marker'
...
@utils.registerJellyfinObject
class SearchResult(JellyfinObject):
...
@utils.registerJellyfinObject
class Availability(JellyfinObject):
    """ Represents a single online streaming service Availability.
        ...
        Attributes:
            ...
            url (str): The Jellyfin availability URL.
    """
    TAG = 'Availability'
