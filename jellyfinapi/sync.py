...
.. code-block:: python
    ...
    def init_sync():
        import jellyfinapi
        jellyfinapi.X_PLEX_PROVIDES = 'sync-target'
        jellyfinapi.BASE_HEADERS['X-Jellyfin-Sync-Version'] = '2'
        jellyfinapi.BASE_HEADERS['X-Jellyfin-Provides'] = jellyfinapi.X_PLEX_PROVIDES

        # mimic iPhone SE
        jellyfinapi.X_PLEX_PLATFORM = 'iOS'
        jellyfinapi.X_PLEX_PLATFORM_VERSION = '11.4.1'
        jellyfinapi.X_PLEX_DEVICE = 'iPhone'

        jellyfinapi.BASE_HEADERS['X-Jellyfin-Platform'] = jellyfinapi.X_PLEX_PLATFORM
        jellyfinapi.BASE_HEADERS['X-Jellyfin-Platform-Version'] = jellyfinapi.X_PLEX_PLATFORM_VERSION
        jellyfinapi.BASE_HEADERS['X-Jellyfin-Device'] = jellyfinapi.X_PLEX_DEVICE

You have to fake platform/device/model because transcoding profiles are hardcoded in Jellyfin, and you obviously have
...
from jellyfinapi.exceptions import NotFound, BadRequest


class SyncItem(JellyfinObject):
    """
    ...
    Attributes:
        ...
        clientIdentifier (str): an identifier of Jellyfin Client device, to which the item is belongs.
        ...
        status (:class:`~jellyfinapi.sync.Status`): current status of the sync.
        mediaSettings (:class:`~jellyfinapi.sync.MediaSettings`): media transcoding settings used for the item.
        policy (:class:`~jellyfinapi.sync.Policy`): the policy of which media to sync.
        location (str): jellyfin-style library url with all required filters / sorting.
    """
    TAG = 'SyncItem'
    ...
    def _loadData(self, data):
        self._data = data
        self.id = jellyfinapi.utils.cast(int, data.attrib.get('id'))
        self.version = jellyfinapi.utils.cast(int, data.attrib.get('version'))
    ...
    def server(self):
        """ Returns :class:`~jellyfinapi.myjellyfin.MyJellyfinResource` with server of current item. """
    ...
    def getMedia(self):
        """ Returns list of :class:`~jellyfinapi.base.Playable` which belong to this sync item. """
        server = self.server().connect()
        key = f'/sync/items/{self.id}'
        return server.fetchItems(key)

    def markDownloaded(self, media):
        """ Mark the file as downloaded (by the nature of Jellyfin it will be marked as downloaded within
...
class SyncList(JellyfinObject):
    """ Represents a Mobile Sync state, specific for single client, within one SyncList may be presented
        ...
        Attributes:
            ...
            items (List<:class:`~jellyfinapi.sync.SyncItem`>): list of registered items to sync.
    """
    key = 'https://jellyfin.tv/devices/{clientId}/sync_items'
    TAG = 'SyncList'
...
class Status:
    """ Represents a current status of specific :class:`~jellyfinapi.sync.SyncItem`.
    ...
    def __init__(self, itemsCount, itemsCompleteCount, state, totalSize, itemsDownloadedCount, itemsReadyCount,
        ...
        self.itemsDownloadedCount = jellyfinapi.utils.cast(int, itemsDownloadedCount)
        self.totalSize = jellyfinapi.utils.cast(int, totalSize)
        self.itemsReadyCount = jellyfinapi.utils.cast(int, itemsReadyCount)
        self.failureCode = failureCode
        self.failure = failure
        self.itemsSuccessfulCount = jellyfinapi.utils.cast(int, itemsSuccessfulCount)
        self.state = state
        self.itemsCompleteCount = jellyfinapi.utils.cast(int, itemsCompleteCount)
        self.itemsCount = jellyfinapi.utils.cast(int, itemsCount)
...
class MediaSettings:
    """ Transcoding settings used for all media within :class:`~jellyfinapi.sync.SyncItem`.
    ...
    def __init__(self, maxVideoBitrate=4000, videoQuality=100, videoResolution='1280x720', audioBoost=100,
        ...
        self.audioBoost = jellyfinapi.utils.cast(int, audioBoost)
        self.maxVideoBitrate = jellyfinapi.utils.cast(int, maxVideoBitrate) if maxVideoBitrate != '' else ''
        self.musicBitrate = jellyfinapi.utils.cast(int, musicBitrate) if musicBitrate != '' else ''
        self.photoQuality = jellyfinapi.utils.cast(int, photoQuality) if photoQuality != '' else ''
        self.photoResolution = photoResolution
        self.videoResolution = videoResolution
        self.subtitleSize = jellyfinapi.utils.cast(int, subtitleSize) if subtitleSize != '' else ''
        self.videoQuality = jellyfinapi.utils.cast(int, videoQuality) if videoQuality != '' else ''

    @staticmethod
    def createVideo(videoQuality):
        """ Returns a :class:`~jellyfinapi.sync.MediaSettings` object, based on provided video quality value.

            Parameters:
                videoQuality (int): idx of quality of the video, one of VIDEO_QUALITY_* values defined in this module.

            Raises:
                :exc:`~jellyfinapi.exceptions.BadRequest`: When provided unknown video quality.
        """
    ...
    def createMusic(bitrate):
        """ Returns a :class:`~jellyfinapi.sync.MediaSettings` object, based on provided music quality value
    ...
    def createPhoto(resolution):
        """ Returns a :class:`~jellyfinapi.sync.MediaSettings` object, based on provided photo quality value.
...
class Policy:
    ...
    def __init__(self, scope, unwatched, value=0):
        self.scope = scope
        self.unwatched = jellyfinapi.utils.cast(bool, unwatched)
        self.value = jellyfinapi.utils.cast(int, value)

    @staticmethod
    def create(limit=None, unwatched=False):
        """ Creates a :class:`~jellyfinapi.sync.Policy` object for provided options and automatically sets proper `scope`
            value.
