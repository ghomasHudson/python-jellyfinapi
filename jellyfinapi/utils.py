...
# Search Types - Jellyfin uses these to filter specific media types when searching.
SEARCHTYPES = {
    'movie': 1,
    'show': 2,
    'season': 3,
...
# Tag Types - Jellyfin uses these to filter specific tags when searching.
TAGTYPES = {
    'tag': 0,
    'genre': 1,
    'collection': 2,
...
# Jellyfin Objects - Populated at runtime
PLEXOBJECTS = {}
...
def registerJellyfinObject(cls):
    ...
    if ehash in PLEXOBJECTS:
        raise Exception(f'Ambiguous JellyfinObject definition {cls.__name__}(tag={cls.TAG}, type={etype}) '
...
def searchType(libtype):
    """ Returns the integer value of the library string type.
        ...
        Parameters:
            libtype (str): LibType to lookup (See :data:`~jellyfinapi.utils.SEARCHTYPES`)

        Raises:
            :exc:`~jellyfinapi.exceptions.NotFound`: Unknown libtype
    """
    libtype = str(libtype)
    try:
...
def tagType(tag):
    """ Returns the integer value of the library tag type.
        ...
        Parameters:
            tag (str): Tag to lookup (See :data:`~jellyfinapi.utils.TAGTYPES`)

        Raises:
            :exc:`~jellyfinapi.exceptions.NotFound`: Unknown tag
    """
    tag = str(tag)
    try:
        return TAGTYPES[tag]
...
def downloadSessionImages(server, filename=None, height=150, width=150,
    ...
    """ Helper to download a bif image or thumb.url from jellyfin.server.sessions.
...
def download(url, token, filename=None, savepath=None, session=None, chunksize=4096,   # noqa: C901
    ...
    """ Helper to download a thumb, videofile or other media item. Returns the local
       ...
       Parameters:
            ...
            token (str): Jellyfin auth token to include in headers.
...
def getMyJellyfinAccount(opts=None):  # pragma: no cover
    """ Helper function tries to get a MyJellyfin Account instance by checking
    ...
    """
    from jellyfinapi import CONFIG
    ...
    if opts and opts.username and opts.password:
        print(f'Authenticating with Jellyfin.tv as {opts.username}..')
        return MyJellyfinAccount(opts.username, opts.password)
    # 2. Check Jellyfinconfig (environment variables and config.ini)
    config_username = CONFIG.get('auth.myjellyfin_username')
    config_password = CONFIG.get('auth.myjellyfin_password')
    if config_username and config_password:
        print(f'Authenticating with Jellyfin.tv as {config_username}..')
        return MyJellyfinAccount(config_username, config_password)
    config_token = CONFIG.get('auth.server_token')
    if config_token:
        print('Authenticating with Jellyfin.tv with token')
        return MyJellyfinAccount(token=config_token)
    # 3. Prompt for username and password on the command line
    username = input('What is your jellyfin.tv username: ')
    password = getpass('What is your jellyfin.tv password: ')
    print(f'Authenticating with Jellyfin.tv as {username}..')
    return MyJellyfinAccount(username, password)


def createMyJellyfinDevice(headers, account, timeout=10):  # pragma: no cover
    """ Helper function to create a new MyJellyfinDevice. Returns a new MyJellyfinDevice instance.

        Parameters:
            headers (dict): Provide the X-Jellyfin- headers for the new device.
                A unique X-Jellyfin-Client-Identifier is required.
            account (MyJellyfinAccount): The Jellyfin account to create the device on.
            timeout (int): Timeout in seconds to wait for device login.
    """
    from jellyfinapi.myjellyfin import MyJellyfinPinLogin

    if 'X-Jellyfin-Client-Identifier' not in headers:
        raise BadRequest('The X-Jellyfin-Client-Identifier header is required.')

    clientIdentifier = headers['X-Jellyfin-Client-Identifier']

    pinlogin = MyJellyfinPinLogin(headers=headers)
...
def jellyfinOAuth(headers, forwardUrl=None, timeout=120):  # pragma: no cover
    """ Helper function for Jellyfin OAuth login. Returns a new MyJellyfinAccount instance.
    ...
    """
    from jellyfinapi.myjellyfin import MyJellyfinAccount, MyJellyfinPinLogin

    if 'X-Jellyfin-Client-Identifier' not in headers:
        raise BadRequest('The X-Jellyfin-Client-Identifier header is required.')

    pinlogin = MyJellyfinPinLogin(headers=headers, oauth=True)
    print('Login to Jellyfin at the following url:')
    ...
    if pinlogin.token:
        ...
        return MyJellyfinAccount(token=pinlogin.token)
    else:
        print('Login failed.')
